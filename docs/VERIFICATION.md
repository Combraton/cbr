...
**Relevance is a stated rule, not a vibe.** A claim is carried when one of its conditions names a repository the basis names, or when its scope or statement shares a term with the question. Everything else is omitted with reason `applicability`, one omission each, so a caller counts what it did not get; a pilot repository with twenty-two decisions does not put all twenty-two into a packet about one of them.
