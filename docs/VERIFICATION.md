...
**Relevance is a stated rule, not a vibe.** A claim is carried when one of its conditions names a repository the basis names, or when its scope or statement shares a term with the question. Everything else is omitted with reason `applicability`, one omission each, so a caller counts what it did not get. **What that rule does on a real repository, measured:** Knowscroll-v2's twenty-two decisions each carry a condition naming the repository the basis names, so the M3d packet carries **all twenty-two**, at 277–280 bytes each. The first limb of the rule is repository-wide by construction, and a repository that keeps every decision in one scope gets every decision. That is the rule working as written, not a defect, and it is recorded here because an earlier version of this sentence used the opposite case as its illustration.
