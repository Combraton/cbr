...
**A citation resolves to an exact span at a named tree.** Each cited file is sealed as an evidence artifact under its blob id, with the tree, the repository and the path as capture anchors. Journey 1 fetches every citation and compares it with `git cat-file blob`.
