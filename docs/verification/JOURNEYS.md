...
**This is not J2's acceptance.** The model is the `model.fake` control, labelled so in every configuration it runs under, and J2's live run on CBR's own test output ([m5 READINESS §10](../work/m5/READINESS.md#10-live-calls-each-planned-estimated-and-capped-before-it-and-each-on-the-owners-word-at-the-time)) has not been run. What a fake can establish is the mechanism: what is sealed, what is sent, what is refused, and what the packet declares.
