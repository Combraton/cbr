...
| Prerequisites and inputs | Synthetic inputs written by the tests: cargo's test output of 8 binaries and 3 failures, a conformance manifest of 160 results with 2 failing, 20,000 bytes that are not text, and a 1.2 MB log. The basis names a repository the provider never registered, so nothing else in the request can ask a model. |
| Implementation basis | m5a, `cbr-context-compiler/3`, projection format `cbr-project-large-result/1`; protocol `v0.1.0`. |
| Model and provider | `simulated (fake)`: `model.fake`, answering `ids:u1` (or `ids:u999` for the refused part) to every part. |
