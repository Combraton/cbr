...
### J6 pilot, Knowscroll-v2: decision memory, no model — **failed**
...
**Scored: FAILED.** Of the three facts a correct packet had to hold, the packet holds **two**, and no trap was triggered. The `git grep` baseline holds none of the three. Which facts were missing is deliberately not recorded: this question runs again, and that rerun has to be fair.

Both pilots therefore beat the plain lexical baseline and neither met its oracle.
...
Both pilots were run at m3d, scored, and **both failed their oracles** — brian2 holding one of three required facts, Knowscroll two of three, neither triggering its trap, and the plain `git grep` baseline holding none of either set. Neither failure was answered by tuning the compiler against the question that produced it.
