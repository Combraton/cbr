...
```sh
git checkout 041ad5f && cargo build --workspace --locked   # the binaries the harness and the proof default to
...
- The path `/private/var/tmp/cbr-j2`, where the inputs were produced, is in the inputs: on seven lines of the red log and five of the green one. Each red baseline packet carries five of them, inside `j2_harness`'s failure blocks, and no other packet carries any. It names no user, machine or volume, which is why the inputs were produced there.
