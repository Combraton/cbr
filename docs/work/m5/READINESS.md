...
7. **A packet the publication guard refuses ends its job with the typed reason `packet_invalid`** — [STATE](../STATE.md) (*Earlier — packet_invalid*) and [VERIFICATION](../../VERIFICATION.md#the-deterministic-packet-compiler-and-what-a-packet-is-compiled-from). Answered in the session as question 12, whose text, this session's, read *"#43's leftover question: a packet the id guard refuses should end its job with a typed `packet_invalid`. I recommend yes."*, by *"10-12 I agree with your recommendation"*. That the request ends `refused` is this session's reading within that ruling, not part of the owner's words.
...
- ~~A pending defect fix, not yet placed in §9's order: a context provider whose tick's batch fails to commit after an evidence peer has sealed a packet leaves the request `preparing` for ever once the clock moves.~~ **Fixed on 2026-09-26 by the peer-capture change**: the capture instant is kept before the send, in a commit of its own, and a kept revision's deadline is judged at it ([STATE](../STATE.md), *This change — a peer's seal survives a lost commit*). The path is conformance-only: `evidence_provider` sits inside the `context` test control, which production refuses.
