...
**The owner's decisions of 2026-09-26.** This session put twelve questions, and the owner answered them in the session in one reply, which is in that session's transcript and not in this repository: *"1. yes lets do 2. yes lets do with recommendation 3. yes 4. yes lets do and close 5-9 sound good , 10-12 I agree with your recommendation"*. Each item below is in this session's wording, and nothing in it beyond the quotes is the owner's own words. The questions were not all put the same way. Questions 1 to 4, 10 and 12 each carried an explicit *"I recommend"*: they are items 8 to 11, 17 and 7, and the reply accepted each. Questions 5 to 9, items 12 to 16, were put as what this session would proceed with unless the owner said no, and the reply answered them *"sound good"*. Question 11, item 18, carried no explicit recommendation, as that item records.
...
- **A pending defect fix, not yet placed in §9's order**: a context provider killed after an evidence peer has sealed a packet and before the tick's batch commits leaves the request `preparing` for ever once the clock moves, because the retry describes the packet with a new capture instant and the peer answers `idempotency_conflict`. It has been there since m3a, was found by the fourth verification of `packet_invalid`, and gets a slice of its own ([STATE](../STATE.md), *This change — packet_invalid*, follow-up 8). Where that slice goes in the order is this session's proposal to make, and the owner's to change.
