...
- **`j2_run.py` stops against 492,204**, its `WORST_CASE_PROJECTION_TOKENS`, which is kept: a bound at or above a projection's worst case, which a test reads back across the boundary and holds there. Tightening it to 341,216 is the owner's call at the next J2 run.
