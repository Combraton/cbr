...
- ~~A pending defect fix, not yet placed in §9's order: a context provider whose tick's batch fails to commit after an evidence peer has sealed a packet leaves the request `preparing` for ever once the clock moves.~~ **Fixed on 2026-09-26 by the peer-capture change**: the capture instant is kept before the send, in a commit of its own, and a kept revision's deadline is judged at it ([STATE](../STATE.md), *Earlier — a peer's seal survives a lost commit*). The path is conformance-only: `evidence_provider` sits inside the `context` test control, which production refuses.
