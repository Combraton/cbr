...
| **m5-arith** | **One worst-case convention, in code**: a question is counted as its first send and its widest repair, computed in one place for every published figure. **Candidate caps on escaped bytes**, 8,192 carried bytes of text and 1,024 of path (item 12), counted as a request body carries them, with a marker where a candidate is cut. **The m4e stop prices a run's selection questions** beside its flow. | m5a-3, whose projection figures it recomputes. Before m5b, by the owner's decision (item 13) | Every published worst case computed by a test under the one convention, and each figure a harness holds read back across the boundary (§8). A candidate at each cap and one byte past it, the cut carrying its marker. The m4e stop refuses a run whose flow and selection questions would pass it. |
