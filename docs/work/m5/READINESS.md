...
- **Both failures are CBR's, and both have the same cause.** These are the only two fixtures whose context script has an `execute` step: the context provider itself submits an investigation to the executor, with `origin` naming its job. CONTEXT §4 requires this (CTX-18, CTX-19): "If preparation needs an execution, the provider submits it as an ordinary, separately authorized caller". CBR's `context.script` performs no `execute`. A step it does not perform holds the job (`_ => break` in `crates/cbr-provider/src/provider/context_ops.rs`), as STATE has recorded since M3a. So the investigation is never submitted and the request never becomes `ready`. **This is CBR acting as an `execution/1` client**, the role [PROTOCOL-PIN](../readiness/PROTOCOL-PIN.md) gives it for PIO-backed investigation. It is not a runner limit, and it is not a reason to serve `execution/1`.
