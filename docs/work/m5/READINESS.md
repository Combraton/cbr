...
|---|---|
| **Journey 2 first**: large search results, test logs and JSON processed under bounded model context, with what was omitted kept explicit (§3) | Replace M4's envelope. By the owner's decision of 2026-09-26 (item 11), m5-settle changes admission and settlement (§9), and nothing else of the envelope changes: the ledger, the ceilings, the credential rule and the recording boundary are M4's and stay exactly as they are. |
...
In the order the owner set: J2, then the loop, then the tools, then the families that need both. **J2 live failed on 2026-09-25**, so m5a-3 and J2's live rerun come before m5b. **The two families added on 2026-09-25 go after m5e and before m5f**, so that maintenance, whose triggers include completed investigations, can start them. By the owner's decisions of 2026-09-26, **m5-arith goes before m5b** (item 13), **m5-settle before m5h's live run** (item 11), and **CBR's context-initiated investigation executions (CTX-18, CTX-19, §7) after m5e** (item 17). The whole order is m5a, m5a-3 and J2's rerun, m5-arith, m5b, m5c, m5d, m5e, CTX-18/19, m5i, m5j, m5f and m5h, with m5-settle before m5h's live run and m5g at any point. Where m5a-3 and the two families sit is this session's proposal and the owner's to change, and so is placing CTX-18/19 straight after m5e, ahead of m5i, m5j, m5f and m5h: that is this session's placement within the owner's "after m5e".
...
| **m5-settle** | **Settlement within what admission reserved**: the four routes by which a settlement can pass its reservation are closed, and so is the freed-room case, amending §1 by the owner's decision (item 11). | M4's admission and settlement. Before m5h's live run, by the owner's decision (item 11) | A test per route and one for the freed-room case, each observed red before the change and green after, with a mutant per route. §1's amendment is the only change to the envelope. |
