...
- **A part's worst send is 41,029 tokens.** That is a part at every bound (`PART_BYTES` of candidates as the body carries them, `PART_UNITS` ids, the widest labels and numbers, and the preamble at its widest), with a task larger than the protocol admits and the longest model name, `MiniMax-M2.7-highspeed`.
- **A part's question is 84,106**: its first send and its widest repair, as [m4 READINESS §3](../m4/READINESS.md#what-model-assisted-discovery-costs-against-the-three-ceilings) prices every question on the serving path (`model::question_worst`).
- **A whole projection's serving worst case is 336,424**: that question for each of `MAX_PARTS` parts.
- **`j2_run.py` stops against 492,204**, its `WORST_CASE_PROJECTION_TOKENS`, which is kept: a bound at or above a projection's worst case, which a test reads back across the boundary and holds there. Tightening it to 336,424 is the owner's call at the next J2 run.
- **Beside discovery's published flow of 517,492, a projection is 853,916**, under the per-job ceiling of 1,000,000, asserted by a test.
- **Not claimed:** a projection beside the flow and one selection question is 1,021,223, over the per-job ceiling, so a request asking all three can be refused by its own job's ceiling at the worst. No test claims it fits, and sizing it is proposed for m5e (§9, the m5-arith row).
