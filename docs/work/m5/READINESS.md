...
**The owner's decisions of 2026-09-26.** This session put twelve questions, and the owner answered them in the session in one reply, which is in that session's transcript and not in this repository: *"1. yes lets do 2. yes lets do with recommendation 3. yes 4. yes lets do and close 5-9 sound good , 10-12 I agree with your recommendation"*. Each item below is in this session's wording. Where an item quotes a question's text or a recommendation, the quote is labelled there as this session's; the only words quoted as the owner's are the reply above, or a part of it. The questions were not all put the same way. Questions 1 to 4, 10 and 12 each carried an explicit *"I recommend"*: they are items 8 to 11, 17 and 7, and the reply accepted each. Questions 5 to 9, items 12 to 16, were put as what this session would proceed with unless the owner said no, and the reply answered them *"sound good"*. Question 11, item 18, carried no explicit recommendation, as that item records.

7. **A packet the publication guard refuses ends its job with the typed reason `packet_invalid`** — [STATE](../STATE.md) (*This change — packet_invalid*) and [VERIFICATION](../../VERIFICATION.md#the-deterministic-packet-compiler-and-what-a-packet-is-compiled-from). Answered in the session as question 12, whose text, this session's, read *"#43's leftover question: a packet the id guard refuses should end its job with a typed `packet_invalid`. I recommend yes."*, by *"10-12 I agree with your recommendation"*. That the request ends `refused` is this session's reading within that ruling, not part of the owner's words.
8. **A loop is started by the `cbr/loop` extension, listed in `requires`**, so a provider that does not understand it refuses the request. Answered in the session.
9. **J4's "deduplication disabled" control is worded as question 2 recommended**: "with the commit's durable identity removed, two findings; with deduplication removed, a typed conflict and no section" — §7 and §9 (m5b). That wording is question 2's recommendation, this session's and not the owner's, which the owner accepted in the session with *"2. yes lets do with recommendation"*.
10. **CBR's own internal commit commands are admitted within the job's resolved view and readable evidence, with no grant minted.** Answered in the session.
11. **m5-settle closes the four routes by which a settlement can pass its reservation, and the freed-room case, and it is done before m5h's live run** — §1 and §9. That is the substance of this session's question 4, in this session's words: closing them changes M4's admission and settlement, which §1 had promised to leave exactly as they were, so §1 now names the change. The owner's reply to question 4 was *"4. yes lets do and close"*. **The freed-room case itself is not in question 4's own text**; it came from an earlier message in the session about m5-settle, and question 4 carries it forward. *(Until `packet_invalid`'s fifth verification this item gave the timing alone.)*
...
|---|---|
| **Journey 2 first**: large search results, test logs and JSON processed under bounded model context, with what was omitted kept explicit (§3) | Replace M4's envelope. Admission, the ledger, the ceilings, the credential rule and the recording boundary are M4's and stay as they are, **except for the one change the owner approved on 2026-09-26 (*"4. yes lets do and close"*), m5-settle** (§9): a count reserves what its settlement can charge; a completion admitted on a count reserves, with its count, no more than the send it counted for; a completion billed input above its count and the margin closes the count path; and a bill above its reservation is an `overrun` naming its job and request, after which the store admits nothing. What a counter holds, the bill, is unchanged. |
...
| **J4** | CBR restarts during model-assisted work and resumes from durable state, with no duplicate commit and no fabricated completion | **Duplicate commit**: kill between the model response and the commit, restart, and require exactly one committed revision; with the commit's durable identity removed, two findings; with deduplication removed, a typed conflict and no section. That control follows the owner's decision of 2026-09-26 (item 9). A job whose checkpoint is intact must not report a finding it never derived. |
...
**What was not established, and m5g's answer (2026-09-26).** Each of these fixtures starts every participant — the evidence provider, the context provider and the executor — from the one participant descriptor under test, so with CBR's descriptor the executor is CBR, which serves no `execution/1`, and the fixture is unsupported. Whether the runner could launch one participant from a different implementation than the others was open. m5g found:

- **The runner itself cannot.** It takes one `--participant` ([`main.rs`](../../../vendor/protocol/v0.1.0/conformance/runner/src/main.rs) line 57, loaded once at line 485), and `start_participant` launches every named participant from that one descriptor (`descriptor: self.ctx.descriptor`, [`exec.rs`](../../../vendor/protocol/v0.1.0/conformance/runner/src/exec.rs) line 1293). The fixture step has `step`, `name`, `config`, `credential_principals`, `within_ms`, `expect`, `capture`, `note` and `session`, with `additionalProperties: false` ([`fixture.schema.json`](../../../vendor/protocol/v0.1.0/conformance/schemas/fixture.schema.json) lines 1245–1291), so a fixture cannot name an implementation either.
- **A descriptor can, because its launch argv is any program.** The runner runs that program once per participant, with that participant's rendered launch configuration. [`cbr-with-reference-executor-unix.json`](../../../conformance/participants/cbr-with-reference-executor-unix.json) launches [`scripts/reference_executor_launch.py`](../../../scripts/reference_executor_launch.py). That script replaces itself with the protocol's reference provider when the configuration carries `executor`, the scripted executor adapter of the pinned [`launch-config.schema.json`](../../../vendor/protocol/v0.1.0/conformance/schemas/launch-config.schema.json) (line 167), and the runner launched it as a named composition participant. An `executor` configuration anywhere else is refused, so the descriptor's `execution/1` claim cannot make a single-participant fixture pass on the reference under CBR's name, and `scripts/run_fixtures.py` refuses the descriptor unless its filter selects composition fixtures only. A configuration without `executor` gets `cbr-provider`. The reference provider is built from the same verified release extraction and lockfile as the runner, by `scripts/build_runner.py --reference-executor`, which records its SHA-256; the launcher refuses a binary that does not match. **No runner change, no protocol change and nothing vendored.** Everything from this route is labelled **reference executor**. It shows CBR's context, evidence and knowledge providers working with a second implementation as the executor, and it is never real-adapter or PIO evidence.
- **The route was checked, not assumed.** First, all fourteen pass against the reference provider alone, so the fixtures can pass on this machine. Second, in the routed run every one of the 11 executor sessions negotiated the reference's limits (1 MiB frames, 256 array items), and every one of the 32 other sessions negotiated CBR's (2 MiB, 4096; `crates/cbr-provider/src/config.rs`).

**The ten, with the reference executor** (runner `v0.1.0`, archive `bde78924…`, the whole `composition.` filter):

| Fixture | Outcome |
|---|---|
| `cancelling-one-consumer-leaves-the-other` | pass |
| `claims-in-packets-block-only-the-required-boundary` | pass |
| `context-provider-outage-advisory-proceeds-required-waits` | pass |
| `correction-before-dispatch-blocks-the-superseded-revision` | pass |
| `executor-fetches-bound-packet-and-seals-outputs` | pass |
| `executor-verifies-assembled-packet-bytes` | pass |
| `missing-required-context-blocks-bound-work` | pass |
| `pinned-and-current-bindings-treat-supersession-differently` | pass |
| `blocked-consumer-releases-the-slot-for-preparation` | **fail**, step 20: `inv-1` is `not_found` at the executor |
| `preparation-runs-without-the-consumer-slot` | **fail**, step 17: `r-2` stays `preparing` |

The three CBR-only fixtures still pass through the same descriptor, so the run is 11 pass, 2 fail and 1 unsupported. Each failure reproduced in two more runs.

- **Both failures are CBR's, and both have the same cause.** These are the only two fixtures whose context script has an `execute` step: the context provider itself submits an investigation to the executor, with `origin` naming its job. CONTEXT §4 requires this (CTX-18, CTX-19): "If preparation needs an execution, the provider submits it as an ordinary, separately authorized caller". CBR's `context.script` performs no `execute`. A step it does not perform holds the job (`_ => break` in `crates/cbr-provider/src/provider/context_ops.rs`), as VERIFICATION has recorded since M3a ([What m3a does not establish](../../VERIFICATION.md#context-requests-packets-and-claims-in-packets)). So the investigation is never submitted and the request never becomes `ready`. **This is CBR acting as an `execution/1` client**, the role [PROTOCOL-PIN](../readiness/PROTOCOL-PIN.md) gives it for PIO-backed investigation. It is not a runner limit, and it is not a reason to serve `execution/1`.
- **No expectation file for this descriptor.** `check_results.py` refuses any `fail` by design, and loosening that rule is not m5g's call. When the context provider submits its investigations, the expectation is 13 pass and 1 unsupported (the verification fixture). Until then the ten are measured and recorded here, not gated.
- **`thirdparty-publisher-feeds-reference-verification` stays blocked.** Its one participant, `evd`, serves `evidence/1` and `verification/1` to the same `owner` session, so no per-participant route can split the two, and routing `evd` to the reference would test nothing of CBR's. It also launches the `minimal-publisher` client, which is not vendored. It needs CBR to serve `verification/1`, which [RELEASE-SCOPE §3](../readiness/RELEASE-SCOPE.md#3-out-of-scope-for-v01-and-why) excludes. So it stays blocked permanently.
- **Journey 7 still waits for PIO.** The reference executor is a protocol peer, not an adapter.
...
- **A part's worst send is 41,628 tokens.** That is a part at every bound (`PART_BYTES` of candidates as the body carries them, `PART_UNITS` ids, the widest labels and numbers, and the preamble at its widest), with a task larger than the protocol admits and the longest model name, `MiniMax-M2.7-highspeed`, in the dialect that frames the most, the OpenAI one: every figure here is the most over every dialect a launch can configure (`model::over_every_dialect`).
- **A part's question is 85,304**: its first send and its widest repair, as [m4 READINESS §3](../m4/READINESS.md#what-model-assisted-discovery-costs-against-the-three-ceilings) prices every question on the serving path (`model::question_worst`).
- **A whole projection's serving worst case is 341,216**: that question for each of `MAX_PARTS` parts.
- **`j2_run.py` stops against 492,204**, its `WORST_CASE_PROJECTION_TOKENS`, which is **kept at 492,204** (this session's decision, at or above the projection worst case of 341,216): a bound at or above a projection's worst case, which a test reads back across the boundary and holds there. Tightening it to 341,216 remains open, for the owner to overturn at the next J2 run.
- **Beside discovery's published flow of 518,628, a projection is 859,844**, under the per-job ceiling of 1,000,000, asserted by a test.
- **Not claimed:** a projection beside the flow and one selection question is 1,027,597, over the per-job ceiling, so a request asking all three can be refused by its own job's ceiling at the worst. No test claims it fits, and sizing it is proposed for m5e (§9, the m5-arith row).

**Changed at m5-arith, 2026-09-26** ([ADR 001 question 17](../../decisions/001-standalone-v0.1-scope-and-stack.md), accepted by the owner 2026-09-26). Until then a part was priced at 41,017, with `MiniMax-M3`'s name, and a whole projection at 492,204, that part sent three times — counted once and repaired once, as discovery's arithmetic then had it — for each part; beside discovery's flow of 373,188 that was 865,392. A count is made only where a send did not fit, so three sends overstated a question, and the live runs used the longer name, so each send was 12 low. m5a's figures were 40,680, 488,160 and 861,348. m5a-3 added the preamble, which says in counts what the floor carries and the room it leaves (at most `PREAMBLE_BYTES`, 142), and lengthened the instruction to ask what to add. Those two changes are the difference between m5a's figures and m5a-3's.

## 9. The pull requests, each with its gate

In the order the owner set: J2, then the loop, then the tools, then the families that need both. **J2 live failed on 2026-09-25**, so m5a-3 and J2's live rerun come before m5b. **The two families added on 2026-09-25 go after m5e and before m5f**, so that maintenance, whose triggers include completed investigations, can start them. By the owner's decisions of 2026-09-26, **m5-arith goes before m5b** (item 13), **m5-settle before m5h's live run** (item 11, **built and landed here**), and **CBR's context-initiated investigation executions (CTX-18, CTX-19, §7) after m5e** (item 17). The whole order is m5a, m5a-3 and J2's rerun, m5-arith, m5b, m5c, m5d, m5e, CTX-18/19, m5i, m5j, m5f, m5-settle and m5h, with m5g at any point. Where m5a-3 and the two families sit is this session's proposal and the owner's to change, and so is placing CTX-18/19 straight after m5e, ahead of m5i, m5j, m5f and m5h: that is this session's placement within the owner's "after m5e".
...
| **m5-arith** | **One worst-case convention, in code, accepted by the owner 2026-09-26** ([ADR 001 question 17](../../decisions/001-standalone-v0.1-scope-and-stack.md), item 12). `model::question_worst` prices a question as its first send and its widest repair, and every published figure is computed through it at the widest bodies, the longest model name among them, as the most over every dialect a launch can configure ([m4 READINESS §3](../m4/READINESS.md#what-model-assisted-discovery-costs-against-the-three-ceilings)). **The m4e stop prices a run's selection questions** as well as its flow. **Candidate caps on escaped bytes**, 8,192 carried bytes of text and 1,024 of path (item 12), counted as a request body carries them, cut with a marker ([m4 READINESS §5](../m4/READINESS.md#what-a-candidate-shows-a-model-and-where-it-is-cut)). `scripts/escape_scan.py` re-runs the survey. **No change to admission, settlement, the ledger or the ceilings** (§1). | M4's runtime; m5a-3, whose projection figures it recomputes. Before m5b, by the owner's decision (item 13) | Every published figure computed by a test from the widest bodies, in every dialect, read back across the boundary (§8). The serving sweep's maximum equal to `question_worst` on every body. A candidate at each cap and one byte past it, the cut carrying its marker. The m4e stop refuses a run whose flow and selection questions would pass it. The escape-dense journey, red at `651c86e` and green. The mutants of its plan, each dying. |
| **m5b** | **The loop and its checkpoints**, over M4's single step, with a fixed read and no tools. Durable checkpoint and turn records; code-enforced termination; resume from checkpoint; the readable-set gate at every door; replay turn by turn. | M4's runtime. It follows m5a by the owner's order, not because it needs anything m5a builds. | **J4's control**: a kill between model response and commit leaves exactly one revision; with the commit's durable identity removed, two findings; with deduplication removed, a typed conflict and no section, following the owner's decision of 2026-09-26 (item 9). Termination by code under a scripted model that never says it is done. The loop's worst case computed against the per-job ceiling. **A loop rebuilt offline reproduces its finding.** The readable-set gate on checkpoints and turn records at fetch, inspect, the listing and replay, both arms of each, with a mutant per door. |
...
| **CTX-18/19** | **The context provider submits its own investigation executions**, the `execute` step of a context script, as an ordinary, separately authorized `execution/1` caller (CONTEXT §4), turning m5g's two failures into passes (§7). | m5e, by the owner's decision (item 17); m5g's descriptor, which measures it | `blocked-consumer-releases-the-slot-for-preparation` and `preparation-runs-without-the-consumer-slot` pass through m5g's descriptor, and the run is 13 pass and 1 unsupported (§7). |
...
| **m5g** | **The composition feasibility check** of §7. Docs, plus a descriptor only if the runner allows one. **Done 2026-09-26:** the runner does not route per participant, a descriptor can; 8 of the ten pass with the reference executor, and 2 fail on CBR's missing `execute` step (§7). | Nothing; it may go at any point | Either the ten execution fixtures run against the reference executor, labelled as such, or the record says why they cannot. |
| **m5-settle** | **Built, and approved by the owner on 2026-09-26** (§1). **Closed with no assumption:** a count reserves the completion body's input bound, where its settlement is capped (route 3); a completion admitted on a count is refused when the count's charge and its own reservation would pass the send the local bound refused, so `question_worst` holds however room is freed; under `Counting::Always` the believed count is capped at the local bound. **Caught, not prevented** (routes 1, 2 and 4, a provider billing above what it was asked): a bill above its reservation is charged whole, recorded as an `overrun` naming its job and request, and ends its call `reservation_overrun`, and the store admits nothing again, read inside `Ledger::admit` before every ceiling, as `bound_unsound` now is; a completion billed input above its count and the margin records `count_unsound`, and the store makes no serving count again. A store can pass a ceiling once, by what calls in flight when the first overrun settles bill above their reservations. A stop is cleared with a new data directory ([ADR 001](../../decisions/001-standalone-v0.1-scope-and-stack.md) question 18). The fake bills within the limits its request declared, and `overbilled:` is the one way a fixture overruns. Also deferred, unchanged: tightening J2's harness bound to 341,216; capping m4e's per-run price at the per-job ceiling. | m5-arith; the owner's ruling | Each rule's test first, with a mutant each. The journey: `model_selection::a_provider_billing_past_its_reservation_stops_the_store_for_good`, a scripted overbill after which `cbr` shows every later request, and one after a restart, refused `reservation_overrun`. |
...
- **A stopped store ends the sequence** (m5-settle): `m4e_run.py` and `j2_run.py` start no further run after one whose store recorded `bound_unsound`, `count_unsound` or `overrun`, and m5h's harness, when written, does the same.
...
**Written before the rerun, decided on 2026-09-26 under the owner's standing grant of 2026-09-25, and run the same day** ([what it spent](#j2-live-rerun-what-it-spent-2026-09-26)) — MiniMax as much as the work needs, with each run's estimate and ceiling written here before it and its spend after, inside CBR's envelope ([STACK §8.1](../readiness/STACK.md)). *(Reworded after the run. As committed before it, in `8d08bef`, the words before this note read: "Written before the rerun, and decided on 2026-09-26 under the owner's standing grant of 2026-09-25 — MiniMax as much as the work needs, with each run's estimate and ceiling written here before it and its spend after, inside ADR 001's account ceilings." Nothing else in this section has changed since, and every figure in it is `8d08bef`'s.)* **Invocation 1's `--run-ceiling` is 492,204 and invocation 2's is 861,357**, the least the harness's stop admits (below); the rerun as a whole is held to **738,306** by the parts the survey found, and is expected to spend about 47,000. Both are far inside the 20,000,000 a five-hour window allows. **Rubric v1.1**, v1's mechanical changes for the `/2` projection, is committed and pushed in the change that runs the rerun, before its first call, and the rerun is scored under v1 as well.
...
*Note, 2026-09-26 (m5-arith).* The worst cases in the J2 sections above are each the convention of its time, which priced every part as three sends: a part at 40,680 and a projection at 488,160 in the first run's plan, and at 41,017 and 492,204 in the rerun's, with every total built from them. They are conservative under m5-arith's convention, in which a question is its first send and its widest repair (§8): a part's question is 85,304, so the rerun's six parts could have held at most 511,824, where 738,306 was allowed. The ceilings, the percentages and the figures above are left as they were computed, and `j2_run.py` still stops against 492,204.
...
- ~~Whether the ten execution fixtures can run against the reference executor.~~ **Settled by m5g on 2026-09-26: they run**, 8 pass and 2 fail on CBR's side (§7).
- **A pending defect fix, not yet placed in §9's order**: a context provider whose tick's batch fails to commit after an evidence peer has sealed a packet, killed there or with the commit failing (SQLite busy past its timeout, a full disk), leaves the request `preparing` for ever once the clock moves, because the retry describes the packet with a new capture instant and the peer answers `idempotency_conflict`. It has been there since m3a, was found by the fourth verification of `packet_invalid` with a kill and reached by its fifth with no kill, and gets a slice of its own ([STATE](../STATE.md), *This change — packet_invalid*, follow-up 8). Where that slice goes in the order is this session's proposal to make, and the owner's to change.
