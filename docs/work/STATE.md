...
## This change — J2's live run, prepared and not run

On `m5/j2-live-prep`, off `main` at `9cd388a`. **No model has been called.** The reviewer's clearance of m5a asked for four things, and the record of each is [READINESS §10](m5/READINESS.md#j2-live-the-inputs-the-estimate-the-stop-and-the-cap-proposed-2026-09-25):
...
- **The estimate, the stop and the cap**, proposed in READINESS, with the models and the inputs by name and digest.
...
- **The merged commit's suite is green, so no input names a failure.** J2 asks which tests failed. READINESS proposes CBR's own output at `f73415d`, m5a's tests-first commit, as an addition for the owner to decide. There, eighteen tests fail, and the dry run named 21 failures: the eighteen, and cargo's three error lines.
