...
The same round found `end_refused_job`'s doc comment claiming more than is true: *"nothing of the refused packet is kept anywhere"*. A probe in which one tick commits the bad section and a later one, at the deadline, publishes it found the section in the ended job's stored record. That record is never served, and nothing of it is logged; the comment now says exactly that (`448b3c2`). This docs commit corrects the rest of what the round found: J4's control, worded in READINESS §7 and §9 and in JOURNEYS as the owner decided (READINESS item 9); item 9's quote, labelled as question 2's recommendation; the line numbers above, cited at `f9980b3`; notes on the m5g and J2 rerun sections below, which were written before the owner's rulings; and three comments, in J, E1 and the compiler section of VERIFICATION, that called `refused` the one terminal state needing no packet, when `cancelled` needs none either and is the caller's.
...
- (b) Every subscriber still `preparing` is refused: `every_request_sharing_a_job_the_guard_refuses_is_refused_with_it`, whose two subscribers share one deadline, and, for one whose deadline is later than the one that brought the packet on, `a_subscriber_with_a_later_deadline_is_refused_with_the_job_an_earlier_deadline_ended`. A subscriber whose own packet would have been valid practically cannot happen: a section dropped from a narrower subscriber's packet for capacity is still named there as an omission (`context::inclusion`), and the guard reads omission ids too.
...
- (d) A valid revision published in the same tick as the refusal is dropped with that tick. Only a script can do this: `a_refusal_commits_nothing_of_the_tick_that_was_refused`.
...
- **W4, a store error swallowed in `end_refused_job`'s caller** (`.unwrap_or(false)` for `?`), survives, and no test here can kill it. It differs only when the store fails while the job is being ended — reading the job or a request, saving a record, or the commit — and nothing in the suite can make a store call fail at that point without fault injection, which the provider does not have. The barrier before the commit can hold the ending and a kill can stop it (T7), but neither makes a store call return an error.
...
- All five were run again in the private copy with T16–T18 in place: each passes `tests/context.rs` (28 of 28), the provider's `context::` unit tests (17) and `expand.rs` (15). The second verification had run each against every `cbr-provider` and `cbr-cli` test binary.
- **An ended job's stored record keeps a section an earlier tick committed**, as the corrected doc comment says. No operation serves a job's record and the log names no id, so nothing of it reaches a reader.
...
At `448b3c2`, with this docs commit's comment edits in place; this docs commit changes comments in two test files and Markdown. The last commit with tests is `6e3f838`, and no provider code has changed since `104615d`: `448b3c2` changes one doc comment.
...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`: 767 tests over 43 `test result` lines, 766 passed, one ignored (the stall harness) and none failed**, at a load average of 4.2 to 5.5; no load flake appeared. By crate: 20, 7, 40, 512 and 188. That is 764 at `f9980b3` plus the three tests of `6e3f838`, T16–T18, all in `tests/context.rs`, which is 28 of 28. At `f9980b3` the same run gave 763 passed and one ignored at a load average of 3.4 to 4.4: 758 at `07bc1ea` plus T10–T15. At `07bc1ea` the same run gave 750 passed, one ignored and seven failed, every one a keychain test on the fake tool's `TimedOut` at a load average of 21 to 31, VERIFICATION's named load flake; run alone straight afterwards, all 19 keychain tests passed. `main`'s count at `651c86e` was 747 over 43, and this change adds twenty: T1–T18 and U1 in `cbr-provider` and J in `cbr-cli`; E1 and E2 are edits.
- **The seven conformance suites**, at `07bc1ea`, and not rerun since: no provider code has changed after `104615d`, and `f9980b3`, `fb39ca1`, `6e3f838`, `448b3c2` and this docs commit change tests, comments and Markdown only. They ran into `mktemp -d` with the worktree's `target/` a symlink for the length of the run and removed afterwards. Six matched their expectations at the first run: `core` 130 and 5 unsupported; `socket` 11 and 2; `evidence` 16; `knowledge` 10; `context` 11; `composition` 3 and 11. `stream`, the first suite of that run, gave 23 pass and one timeout, `stream.control-methods-not-served` at step 1, *no frame within 4999 ms*, at a load average near 20. That was the first fixture on the first launch of a freshly built provider, under load; run again alone straight afterwards, the suite passed 24 of 24 and matched its expectation. It has been observed twice, here and in the independent verification's run of the suites, each time on that first fixture at the first launch of a freshly built provider, under load, and each time, as reported, the suite matched its expectation when rerun alone. No fixture reaches the guard.
