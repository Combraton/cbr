...
At `2819598`, the last commit with tests, with this docs commit's edits in place; this docs commit changes Markdown only. Provider code changed in `5891542`, the fourth fix round's fix, and nowhere else since `104615d` but `448b3c2`'s doc comment; `6deafa9` and `2819598` change `tests/context.rs` alone.
...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`, after `cargo build --workspace --locked`: 781 tests over 43 `test result` lines, 780 passed, one ignored (the stall harness) and none failed**, at a load average of 8.1 to 8.7; no load flake appeared. By crate: 20, 7, 40, 526 and 188; `tests/context.rs` 42 of 42. `2819598` adds no test: it bounds two loops inside existing ones. At `6deafa9`, with `48104d4`'s docs in place, the same run gave the same 781, 780 passed and one ignored, at a load average of 3.9 to 4.8: 776 at `a98235c` plus the five tests added after the fourth verification, T28–T32, all in `tests/context.rs`; the crash row's test, T26 and T27 are extended. At `a98235c`, with `57e6e1d`'s docs in place, the same run gave 775 passed and one ignored at a load average of 3.9 to 5.4: 771 at `fa6b3bf` plus T23–T27. At `fa6b3bf`, with `7119cd5`'s docs in place, the same run gave 770 passed and one ignored at a load average of 3.9 to 6.1: 767 at `448b3c2` plus T19–T22. At `448b3c2`, with `a21a7ae`'s comment edits in place, it gave 766 passed and one ignored at a load average of 4.2 to 5.5: 764 at `f9980b3` plus T16–T18. At `f9980b3` it gave 763 passed and one ignored at a load average of 3.4 to 4.4: 758 at `07bc1ea` plus T10–T15. At `07bc1ea` it gave 750 passed, one ignored and seven failed, every one a keychain test on the fake tool's `TimedOut` at a load average of 21 to 31, VERIFICATION's named load flake; run alone straight afterwards, all 19 keychain tests passed. `main`'s count at `651c86e` was 747 over 43, and this change adds thirty-four: T1–T32 and U1 in `cbr-provider` and J in `cbr-cli`; E1 and E2 are edits.
