...
- `661b598` **pins and scripts**: `m4e_run.py` prices a run at 517,492 + wants × 167,307, reports both figures and each run's price, and prints the stop in force; `j2_run.py`'s constant stays 492,204, and its comments now call it a bound at or above a projection's 336,424.
...
- The docs commit after them, which holds this section.

**The figures, each computed by a test** from the widest bodies — the longest model name, a task past the protocol's, every candidate at the cut, seven-digit lines, every term at its bound:

| | One send | One question |
|---|---:|---:|
| Selection | 83,397 | 167,307 |
| Discovery's terms step | 63,485 | 129,018 |
| Discovery's choice step | 193,213 | 388,474 |
| Discovery's flow; six flows | | 517,492; 3,104,952 |
| A J2 part; a whole projection; beside the flow | 41,029 | 84,106; 336,424; 853,916 |
| One m4e run of 1, 2 and 3 wants | | 684,799; 852,106; 1,019,413 |

The whole of m4e is now bounded at 2,565,201 for future runs (the stop less one want's run, plus a job), and 2,876,812 stays the bound on the three recorded runs, which the old stop admitted. m5b's room beside the flow and one selection question would be 315,201. **Not claimed:** a projection beside the flow and a selection question is 1,021,223, over a job's ceiling, and sizing it is left to m5e.

**What the figure assumes, stated where it is used.** A question holds at most its first send and its widest repair when bills are within their reservations and no room is freed on the refusing counter while a count is in flight. With room freed, an attempt can hold up to its count's reservation more, and a test pins it. Under `Counting::Always` nothing from the body bounds a question. Every ceiling holds at admission, and settlement can pass one by what an admitted call was billed over its reservation, on four routes, each a tested `divergence` ([m4 READINESS §3](m4/READINESS.md#what-model-assisted-discovery-costs-against-the-three-ceilings)).
...
- **The serving sweep**, where each body's maximum equals `question_worst`: prose-wider synthetic 2,812 over 280 runs, truncation-wider 14,786 over 280; terms 129,018, choice 388,474, selection 167,307 and a J2 part 84,106, over 36 runs each.
- **The escape-dense journey**: the choice reserved 142,541 of 193,213; 16 candidates cut, 2 at the path, 4 shown whole; ledger 12,071, 11,337 and 142,541, all `admitted_local`; every cited span its full bytes.
...
- **The m4e dry run** with two wants and `--stop-tokens 852105` stops with "the 852106 worst case of one flow and 2 selection questions", and starts at 852,106.
...
3. Move its turn body to the longest model, and re-pin the turn, the loop and its room, which becomes 315,201; about 51,586 and 206,344 for the turn and the loop, derived and not run. Keep `LOOP_MAX_CALLS = 16`: four sends a question is still a true count of sends.
...
3. **J2's harness bound**: keep 492,204, or tighten it to 336,424 at the next J2 run.
...
6. **m5e**: a projection, the flow and a selection question in one job is 1,021,223, over the job's ceiling, and m5e has to size it.

**Next.** An independent verification of the head, then a pull request, which this stage does not open. m5b rebases onto this only once it has merged.
