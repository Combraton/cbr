...
On `fix/budget-insufficient`, off `main` at `4dcfb04` (#47, `packet_invalid`). Commits: `94da152` the tests, red; `17795f1` a refactor with no change in behaviour; `78ea63f` the fix (A); `05dd265` the join guard (B); `456695d` the cancel rule (C); `6831d62` four tightened tests for what the mutants reached, below; and `279a8fb` the docs; then, after independent verification, `83be67e` four tests for what its mutants reached and the docs [after it](#after-independent-verification); then `main` merged in at `d1bcc00`, bringing #48 (m5-arith) and #49 (peer-capture), [below](#the-merge-of-main). **No model has been called.**
...
- **A, the fix.** `compile` returns its steps unchanged. After compiling, `advance` counts the required items' section bytes (`context::mandatory_size`, §12's count) and refuses each subscriber still `preparing` whose own capacity cannot hold them: `refused`, `budget_insufficient`, `needed`, each required item `unmet` and each advisory one `degraded` with that reason, one revision, and `context.request.changed {state, job, reason}`. No packet is sealed or published for it. The job ends `budget_insufficient` only when the refusal leaves no subscriber `preparing`, with `context.job.ended` after the requests' changes. The tick commits as any other, so the source and derivation artifacts that compiling sealed are kept: no *packet* is sealed, which is not the same as nothing. `end_refused_job`'s per-request body is now `refuse_request`, which both paths use.
- **B, the join guard.** A joined request is prepared by the job's script (§12), but submit checked its capacity against its own script, usually empty: at `4dcfb04` a capacity-11 request joined a job with a 12-byte required section (T3), and the plan's probe saw it published `ready`. A request now joins only when the job script's mandatory size fits its own capacity; otherwise it gets a job of its own (§4: joining is a MAY).
- **C, cancel.** A refused subscriber stays in its job's `requests`, for the events of its job (§10). So `job_continues` now counts only the other subscribers not `refused`: with A alone, cancelling the last preparing subscriber after a refusal answered `true` and left the job running for nobody (T4).

**Choices the owner may overturn, each pinned by a test.**

- **Per subscriber, not the whole job.** In a shared job each subscriber is decided by its own capacity, and the job goes on for the others (T2). `packet_invalid` refuses every subscriber instead (its choice (b), in *Earlier — packet_invalid* below). The difference is deliberate: `output_capacity` is a request's limit (§1), §3's rule is about a request, and §4 says "The job continues while any other request still needs it"; `packet_invalid` fails closed on what in production is a defect, where a capacity too small is an ordinary limit.
- **Two shapes of `budget_insufficient`, unified later.** Refused after compiling: its `job`, items ended `unmet` or `degraded` with reason `budget_insufficient`, and `needed`. Refused at submit, unchanged: no `job`, items `pending`, and `needed`. The fixture pins the submit shape's state, reason, `needed` and packets, not its items. Follow-up 5 of `packet_invalid` (`refused` after `preparing`, with a `job`) now covers this reason too, and §3's own "ends `refused`" supports it; nothing is filed.

**Red, then green.**

| Test | Red at `94da152`, the tests over `4dcfb04`'s code |
|---|---|
| J `a_required_item_its_capacity_cannot_hold_is_refused_with_the_size_it_needs_through_cbr` (`journey_one.rs`) | `tight`, capacity 196 and needing 197, ends `unmet` with `packet.tight.1` |
| T1 `a_compiled_request_whose_required_content_cannot_fit_is_refused_with_the_size_it_needs` | `r-tight`, capacity 490 and needing 491, ends `unmet` with `packet.r-tight.1` and no `needed` |
| T2 `in_a_shared_compiled_job_only_the_subscriber_whose_capacity_cannot_hold_the_required_content_is_refused` | the narrow subscriber, at 490, one byte under the size it needs, is published `partial` with its required item satisfied |
| T3 `a_request_never_joins_a_job_whose_required_content_its_capacity_cannot_hold` | `r-narrow`, at 11, joins `r-wide`'s job |
| T4 `cancelling_the_last_subscriber_a_refusal_left_ends_the_job` | fails at its premise, since nothing refuses the narrow subscriber; with A alone, `job_continues` is `true` |

A turned T1, T2 and J green, B T3, and C T4. T26 of `packet_invalid` now ends its job as `investigation_budget_exhausted`, since a compiled job that cannot fit no longer ends through `finish`.

**Mutants, in a private copy only**: `git archive 456695d` with an `origin` remote added, each mutant applied by script, `tests/context.rs` and J run, and the copy restored. None was applied in the worktree.

- **The plan's**, MA1–MA14 (MA14 as two), MB1–MB3 and MC1: all killed. MA1–MA13, MB1–MB3 and MC1 each fail a test the plan named. MA14 fails `packet_invalid`'s tests, and MA10 and MA11 fail them too, since the two refusals share `refuse_request`.
- **MA15 survives, as the plan expected**: it ends the job whenever none is left `preparing`, even with no refusal. That differs only for a scripted `compile` step after a publication, a test control outside §12.
- **Ten of this change's own**: five were killed at once. Four survived the first run and are killed since `6831d62`:
  - MX1, cancel counting only `preparing` subscribers, by T5, `cancelling_one_subscriber_leaves_the_job_running_for_one_published_under_updates`;
  - MX2, the refusal naming a job after the request, by T2's events;
  - MX4, the ended job left `running`, and MX5, stored with another reason, by T1's read of the stored job.
- **MX6 survives, as equivalent**: it drops the `preparing` check before refusing. In production every subscriber is `preparing` when a job compiles, since compiling is a job's first step and a cancelled subscriber leaves `requests`.
- One run of MX5 also failed a peer test on *the provider never listened*, a load flake, which was not counted.

**Gates at `6831d62`**, with these docs in place:

- **Static gates, all exit 0:** `check_docs.py`, `verify_pin.py`, `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo build --workspace --locked`, and `git diff --check origin/main...HEAD`. No machine path is committed. `Cargo.lock` and `vendor/` are unchanged.
- **`main` moved during this work**: #48, m5-arith, merged as `25eefca`. Against it, `git merge-tree --write-tree origin/main HEAD` merges the code by itself; m5-arith's one line in `context_ops.rs` is in retrieval, apart from this change. Two docs conflict: the top of STATE and VERIFICATION's count row. `main` is to be merged in, not rebased onto, since this record cites its commits, and the merge needs a whole-workspace run of its own.
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`: 789 tests over 43 `test result` lines.**
  - 783 passed, one ignored (the stall harness), and five failed. All five are keychain tests on the fake tool's `TimedOut`, VERIFICATION's named load flake, at a load average of 12.6 to 18.6. Run alone straight afterwards, the provider's 444 unit tests passed, the 19 keychain tests among them.
...
### The merge of `main` (#50, m5-settle)
...
- **Verification round 1.** The code verifier's probes showed that an overrun whose settlement did not land left no stop: when the store refused the settlement (P1, another writer holding the lock) or the process was killed at `model.completion.during_reconciliation` (P4). Now the process keeps an overrun the store refused, refuses every later admission on that store, on any connection, writes it at the first admission the store takes, and charges the call the bill. `model_calls` names the reservation each exchange was sent under, and a start settles a reservation still standing to its recorded bill when that is above it, which writes its `overrun`, before anything is admitted. A settlement lands once. The tests came first, four red for their own reasons (P1, P4, a settlement landing twice, and the journey `a_provider_killed_before_it_settled_an_overbill_is_stopped_when_it_starts_again`); tests for the verifier's four surviving test-gap mutants (V1, V3, V23, V16b) were added. Mutants, each applied alone in a private archive copy: 20 of 20 killed, those four and sixteen of this round's own, each by a test that passed at the baseline; the keychain `TimedOut` load flake fired in two runs and was not counted. Whole workspace, once, through the two-slot wrapper, at `cf9be2b`: 803 tests over 44 result lines, 795 passed, one ignored, seven failed, each the keychain fake tool's `TimedOut` under the mutant run's load; the `cbr-provider` unit binary then passed alone, 484 of 484. The seven standard suites each matched their expectation. Not closed: a call killed before its answer was written (question 18).
- **Verification round 2.** The delta check's probes: an admission already waiting on the lock was granted after the process kept an overrun (C); the live-run harnesses read a killed launch's store before any start had reconciled it, so an overrun only the dead process knew of was missed and the next run started (A); and a start charged a 429 whose body reported usage above its reservation, which the live path settles to nothing (B). Now the process's stop is read under the admission's lock; `cbr-provider --reconcile-ledger` runs a start's reconciliation and exits, and both harnesses run it after each launch and before reading, going no further when it fails; and a record names no reservation when its answer was read as the provider's quota being gone. Tests first, red at `8a938c8` for their own reasons (C, A in each harness, B, and the harness's refusal); tests for the checker's surviving test-gap mutants N1, N2 and N6 were added. Mutants, each applied alone in a private archive copy: 13 of 13 killed, those three, a stricter N1, and nine of this round's own, each by a test that passed at the baseline; the keychain `TimedOut` load flake fired in three runs and was not counted. Whole workspace, once, through the two-slot wrapper, at `9438d5c`: 811 tests over 44 result lines, 810 passed, one ignored, none failed; 539 in `cbr-provider`, 205 in `cbr-cli`. No conformance suite was run: nothing outside the ledger, the recording and a new administration flag changed. Not closed: what no record holds (question 18).
- **Verification round 3.** The checker's deterministic probe: an admission waiting on a refused settlement's own write lock was granted once the ROLLBACK freed it, before the process kept the overrun. Now the overrun is kept before the ROLLBACK, and `settle` says whether it kept it, so `finish` no longer re-reads a list another worker's admission may have emptied. Tests first (the probe, red at `4199b88`), and tests for the checker's surviving gaps C1 (two standing overruns reconciled) and C4 (`--reconcile-ledger` exits non-zero when reconciling fails, and the harness stops). Mutants in a private archive copy: 5 of 5 killed (W1 kept after ROLLBACK, W2, W3, C1, C4); keychain `TimedOut` flakes not counted. Whole workspace, once, through the two-slot wrapper, at `b6e65f6`: 814 tests over 44 result lines, 813 passed, one ignored, none failed; 541 in `cbr-provider`, 206 in `cbr-cli`.

- **Merge.** `23016b1` merges `origin/main` at `25eefca` (#47 `packet_invalid`, #48 m5-arith) into this branch, docs-only conflicts (ADR 001, this file, both READINESS files, VERIFICATION), no code conflict. Gates re-run at the merge head: `check_docs.py`, `verify_pin.py`, `cargo fmt`, `cargo clippy -D warnings`, `cargo build` all clean; `git diff --check origin/main...HEAD` clean; no machine path; `git merge-tree --write-tree origin/main HEAD` clean. Whole workspace, once, through the two-slot wrapper, at `23016b1`: **850 tests over 44 result lines, 848 passed, one ignored, one failed** — `idle_connections_keep_at_least_half_of_command_throughput`, the named load flake, not counted, passed alone in 12.87s; 575 in `cbr-provider`, 206 in `cbr-cli`. No new test: the merge brought in `packet_invalid`'s and m5-arith's, already counted on `main`.

**Commits:** `289079f` tests first; `8d700eb` budget; `6c499fc` model; `57c1a3d` the pre-existing reasons; `7b38363` the fake; `5247cd3` the scripts; then the docs; then the merge `23016b1`.

**What remains** (question 18): a store can pass a ceiling once, by what the calls in flight when the first overrun settles bill above their reservations — at most `work::CONCURRENCY` (2) pool units, plus any unit detached at its deadline and still running. Route 1's excess is at most `local − floor − 1,024` a call, 165,527 at most on the published bodies; routes 2 and 4 have no bound CBR can derive. A settlement lands at its own instant, so a call whose flight crosses a month boundary or lasts past five hours can charge a counter its admission never checked. A call killed before its answer was written leaves nothing for a start to reconcile, and only an overrun is kept and reconciled, not a `bound_unsound` or `count_unsound` note.
...
- **Whole workspace**, once, through the two-slot wrapper, at `ab87380`: **792 tests over 44 result lines, 790 passed, one ignored, one failed**: `ingest_and_fetch::ingested_bytes_fetch_identically_after_sigkill_and_restart` read the socket at 0o755 before the provider's chmod to 0o600 landed, a race outside m5-settle's code, not counted; it passed alone three times. 525 in `cbr-provider`, 200 in `cbr-cli`.
...
**Merge with origin/main (#49, peer-capture fix).** This branch merges `origin/main` at `d1bcc00` (#49, "a peer's seal survives a lost commit", the fix for packet_invalid's follow-up 8) in, docs-only conflicts (this file, `docs/VERIFICATION.md`); `context_ops.rs` auto-merged with no conflict (see *Earlier*, below, for what #49 changed there). Resolved by keeping this section on top and moving #49's own section, below, to *Earlier*. Gates re-run at the merge head `1a448b5`: `check_docs.py`, `verify_pin.py`, `cargo fmt`, `cargo clippy -D warnings`, `cargo build` all clean; `git diff --check origin/main...HEAD` clean; no machine path; `git merge-tree --write-tree origin/main HEAD` clean. Whole workspace, once, through `wslot`, at `1a448b5`: **864 tests over 44 result lines, 862 passed, one ignored, one failed** — `idle_connections_keep_at_least_half_of_command_throughput`, the named load flake, not counted, passed alone in 13.02s; 589 in `cbr-provider`, 206 in `cbr-cli`. No new test: the merge brought in #49's fourteen, already counted on `main`. `context_ops.rs` changed in the merge (#49's fix), so the `context` and `composition` conformance suites were re-run, with the worktree's `target/` a symlink to its build directory only for the run: both match their expectations — `context` 11 of 11 pass; `composition` 3 of 14 pass, 11 unsupported by name.
...
- **Fix A: every peer-bound capture instant is kept before any send, in a commit of its own.** `keep_outgoing_captures` (`crates/cbr-provider/src/provider/context_ops.rs`) runs once `advance` has returned, so after the guard has seen every packet of the tick, and before `seal_packets`. For each packet bound for the peer with no instant kept, it sets `captures[artifact]` to the tick's instant in the in-memory job and commits the same to the stored job, in one provider batch with no event. A failure there fails the tick before anything is sent. CORE §6.3 item 3 asks for this shape: a caller *"persists the command, including the generation, before sending, so a retransmission after a crash carries the original generation"*. The old mechanism, which kept instants only after a send that failed, is removed: `Tick::captures`, `NotSealed::captures`, and the NotSealed arm's `keep_captures` call.
- **Fix D: a kept revision's deadline is judged at its capture instant.** `publish_one` judges `past_deadline` at the instant kept for the revision it is composing, and at the tick's instant when none is kept; an explicit reason still wins. Without it, a retry past the deadline composed the revision with `deadline_passed` for an item the first attempt composed `unmet` for `unavailable`, so the bytes changed and the peer refused every retry. With it, the revision published after the deadline is the one composed at its `captured_at`. CONTEXT §3 allows either reason, and the item is never `satisfied` (CTX-7).
- **A barrier**, `context.packet.after_peer_sealed`, declared in no descriptor: after `seal_packets` has sealed every packet of the tick at the peer, before the batch commits. It sits after the loop, because a barrier holds only once per process.

**Red, then green.** Red at `8608ee4` (`main` plus the barrier), each for its own reason:

| Test | Red at `8608ee4` | After A (`5d88422`) |
|---|---|---|
| `a_kill_after_the_peer_seals_and_before_the_tick_commits_publishes_the_revision_once_the_clock_has_moved` (the fourth verification's kill probe) | *r-1 never left preparing within 50 polls; … packet packet.r-1.1 not sealed at the evidence provider (refused: idempotency_conflict)* | green |
| `a_commit_that_fails_after_the_peer_seals_publishes_the_revision_once_the_clock_has_moved` (probe P2, timed by the barrier, then `BEGIN IMMEDIATE` from a second connection) | the same `idempotency_conflict` | green |
| `a_tick_that_cannot_keep_its_capture_instants_sends_nothing_to_the_peer` (P2's shape: the lock held from before the tick) | *packet.r-1.1 reached the evidence peer* | green |
| `a_kill_after_a_tick_seals_two_packets_at_the_peer_publishes_both_once_the_clock_has_moved` | *r-first never left preparing*, `idempotency_conflict` | green |
| `a_kill_whose_restart_is_past_the_deadline_publishes_the_revision_as_composed_before_it` | `idempotency_conflict` | still `idempotency_conflict`; green after D |
| `a_seal_that_timed_out_before_the_deadline_is_published_after_it_as_composed_before_it` (no kill) | `idempotency_conflict` | still `idempotency_conflict`; green after D |
| Edit: `a_tick_whose_second_peer_send_fails_keeps_both_capture_instants_and_both_replay_after_the_clock_moves` | step 1's premise, *CBR kept nothing of the killed tick*, becomes *both instants kept before the first send*, an intended behaviour change: red at `:74`, *no captures along ["captures"]* | green |
| Edit (a guard, green before and after): `no_packet_of_a_refused_tick_that_publishes_twice_reaches_the_evidence_peer` | the refused job's stored record keeps no capture | green |

`3899885` corrects `item_of`, which read `reason` on an item that has none; both deadline tests failed at the settle, before that line, on every earlier head.

**Mutants.** Eighteen, applied by a script in a private `git archive` copy, each run against `tests/context.rs`. The plan's ten are killed: M1 no `keep_outgoing_captures` call, M2 the instants committed after the sends, M3 only the first packet's instant kept, M4 the instants in memory only, M5 the send ignores the kept instant, M6 an instant already kept overwritten, M7 D removed, M8 D reading the wrong revision's instant, M9 the instants kept before the guard has seen the tick (by the E2 guard), and M10 a failed capture commit ignored and the sends made (by `a_tick_that_cannot_keep_its_capture_instants_sends_nothing_to_the_peer` alone). Of this change's own eight, two were killed at once (the instant overwritten in memory only; the instants keyed by request) and five by the tests of `0397f76`, each shown in the copy:

| Mutant | Killed by |
|---|---|
| Instants kept only for a job with none kept | `a_kill_after_the_peer_seals_a_later_revision_publishes_it_once_the_clock_has_moved` (*r never had 2 revisions … idempotency_conflict*) |
| Instants committed, not set in the in-memory job | the same test: the successful tick's job save drops revision 1's instant |
| D reads revision 1's instant | `a_later_revision_first_composed_past_the_deadline_reads_deadline_passed` (`unavailable` for `deadline_passed`); it kills M8 too |
...
- **A send that timed out, sealed by the peer later, and then a refusal (probe P1) is the peer's state, which CBR cannot roll back.** EVIDENCE §4: *"A sealed, held, purge-pending, purged or already abandoned artifact is never abandoned, so abandon is never a way to discard sealed or held content; that needs `evidence.purge` (§9)."* EVIDENCE §8 gives `evidence.publish` *"prepare, append, seal and abandon on covered artifacts"* and `evidence.purge` *"purge of a covered artifact"* as a separate right; purge also needs the optional feature `evidence.retention_control` (§1). The grant the context provider holds in the pinned composition fixture, and in CBR's tests, is `evidence.publish` alone. Publishing that revision before refusing would reopen choice (d) of packet_invalid, which is the owner's. Fix A does not widen the case: a refused tick keeps no instant and sends nothing.
- **Not covered:** a retry whose bytes change for a reason other than the deadline, such as a model-assisted compile after a kill, would still meet `idempotency_conflict`.
...
- **`main` moved during this change**, from `4dcfb04` to `25eefca` (#48, m5-arith), and has been merged in (below). `git merge-tree --write-tree` merged `0397f76`, the last commit with code or tests, into `25eefca` with no conflict; only these two documents conflicted, and were resolved by hand. #48's change to `context_ops.rs`, a discovery bound (`rows: crate::selection::CANDIDATES`), does not touch this change's paths, and `context_ops.rs` auto-merged.
- **`main` merged into this branch**, a merge commit off `cfc26b4` (this branch's own head before the merge) and `25eefca` (`main`). Auto-merged everywhere but these two documents; `git merge-tree --write-tree origin/main HEAD`, run again after the merge commit, shows no conflict.
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`, re-measured after merging `main` in: 824 tests over 44 `test result` lines, 823 passed, one ignored (the stall harness), none failed**, with `idle_connections_keep_at_least_half_of_command_throughput` passing this run. By crate: 20, 7, 40, 560 and 196; `tests/context.rs` 58 of 58. `main` at `25eefca` (#48) had 810 over 44 (per its own #48 record); this branch's fourteen tests, ten from the fix and four from the second verification round, all in `tests/context.rs`, and `expand.rs`'s J is `main`'s own from `packet_invalid`.
- **The seven conformance suites**, with the worktree's `target/` a symlink to its build directory only for the run, the runner built by `build_runner.py --offline`, all matching their expectations at the first run, re-run after the merge: `stream` 24; `core` 130 and 5 unsupported; `socket` 11 and 2; `evidence` 16; `knowledge` 10; `context` 11; `composition` 3 and 11 unsupported, `packets-are-sealed-at-a-separate-evidence-provider` among the three passes.
- **Merged `main` into this branch, `38c176e`.** `main` had moved to `25eefca` (#48, m5-arith, merged after packet_invalid); the merge is a merge commit, not a rebase, since this section's own hashes above are cited by hash. `context_ops.rs` auto-merged with no conflict: m5-arith's change is a discovery bound and does not touch this change's paths. Only these two documents conflicted, both here and in `docs/VERIFICATION.md`; resolved by keeping this section on top and moving m5-arith's own section, below, to *Earlier*.

## Earlier — m5-arith: one worst-case convention, and the candidate cut

On `m5-arith/convention`, off `main` at `651c86e` (#46), now merging `main` at `4dcfb04` (#47, `packet_invalid`). **Not pushed.** Its ADR question was proposed when this section was written and is **accepted by the owner as of 2026-09-26** ([m5 READINESS](m5/READINESS.md) item 12); the order question (m5-arith before m5b) is likewise decided (item 13). **No model has been called**, and admission, settlement, the ledger and the ceilings are unchanged, as [m5 READINESS §1](m5/READINESS.md#1-scope-and-the-promise) promises. The plan is a judge's synthesis of three planning agents' plans, checked against the code at `2f8c666`, and is in the session's scratch outside this repository; what it decided is recorded here, in [m4 READINESS §3 and §5](m4/READINESS.md#what-model-assisted-discovery-costs-against-the-three-ceilings), in [m5 READINESS §8 to §10](m5/READINESS.md#8-every-new-bound-has-its-arithmetic-computed-by-a-test-and-a-fixture-that-reaches-it) and in [ADR 001 question 17](../decisions/001-standalone-v0.1-scope-and-stack.md), accepted.

**Why.** An audit of the published worst cases found three faults. Discovery priced every step as three copies of one send, on a candidate's raw bytes and `MiniMax-M3`'s name; selection's 39,612 was in no test; and the m4e stop priced a run at one flow and no selection question. Raw bytes understated a body, which is JSON and carries a C0 control in six bytes: a fixture of twenty such spans made a choice that reserved 402,821 tokens, and the per-request ceiling refused it. Three sends overstated a question, because a count is made only where the send did not fit. And the live runs used `MiniMax-M2.7-highspeed`, whose name is 12 bytes longer.

**The commits**, stage by stage, none squashed:

- `9471697` **tests first**, with stubs. Each test was observed red for its own reason, and the escape-dense journey was red at `651c86e` itself: the choice was refused `request_over_ceiling` at 402,821.
- `565e7ac` **the convention, byte-neutral**: `budget::reservation`, `model::repaired` (the one place a repair is built, and what `Runtime::ask` now sends), `model::send_worst`, `model::question_worst`, `wire::response::REPAIRABLE`, `model::SERVING_COUNTING` named at both serving launches, and the test-only `model::harness`. The `Runtime::call` and `budget.rs` documentation now says when a count is made: only after the local bound is refused on the job, the window, the month or the run.
- `cbb6303` **the cut**: `wire::request::carried_bytes` and `carried_within`, and `selection::shown`, the one frame every step shows a candidate through, within 8,192 bytes of text and 1,024 of path as a body carries them, with `[cut]` past either.
- `661b598` **pins and scripts**: `m4e_run.py` prices a run at the flow + wants × a selection question, reports both figures and each run's price, and prints the stop in force; `j2_run.py`'s constant stays 492,204, and its comments now call it a bound at or above a projection's. Its figures were the Responses dialect's, and the verification round below moved them.
- `6c763ea` **tests first for `scripts/escape_scan.py`**, and `995e100` **the script**, the survey that sized the cut, re-runnable at any commit.
- `f3d203b` **the docs**, which began this section.
- `dc2cedf` **tests first for the verification round**, with stubs, and `b2f9a77` **its fixes**; then the docs commits that record it.

**The figures, each computed by a test** from the widest bodies — the longest model name, a task past the protocol's, every candidate at the cut, `selection::CANDIDATES` of them for one item, seven-digit lines, every term at its bound — and each the most over every dialect a launch can configure, which is the OpenAI dialect's on every body:

| | One send | One question |
|---|---:|---:|
| Selection | 83,620 | 167,753 |
| Discovery's terms step | 63,718 | 129,484 |
| Discovery's choice step | 193,548 | 389,144 |
| Discovery's flow; six flows | | 518,628; 3,111,768 |
| A J2 part; a whole projection; beside the flow | 41,628 | 85,304; 341,216; 859,844 |
| One m4e run of 1, 2 and 3 wants | | 686,381; 854,134; 1,021,887 |
...
**Evidence at the head, each run by an agent of this session:**

- **The serving sweep**, where each body's maximum equals `question_worst`, in the Responses dialect, the only one counted: prose-wider synthetic 2,812 over 280 runs, truncation-wider 14,786 over 280; terms 129,018, choice 388,474, selection 167,307 and a J2 part 84,106, over 36 runs each. Those are the Responses dialect's own figures, each below the published one, which is the OpenAI dialect's, where nothing is counted and a question holds its sends.
- **The escape-dense journey**: the choice reserved 142,541 of 193,548; 16 candidates cut, 2 at the path, 4 shown whole; ledger 12,071, 11,337 and 142,541, all `admitted_local`; every cited span its full bytes.
- **Byte identity** of the unified frame: 272,525 windows of the tree, every tracked file's identical; every fake-model journey and golden digest unchanged.
- **The m4e dry run** with two wants and `--stop-tokens 854133` stops with "the 854134 worst case of one flow and 2 selection questions", and so does `--run-ceiling 854133`, and it starts at 854,134; the test now asserts both messages.
- **Mutants**, each applied alone and counted killed only when a test that passed at the baseline failed: 24 of 24 against the tests-first commit's throwaway implementation; 24 of 24 at the convention; 21 of 21 at the cut; 14 of 14 Rust and 12 of 12 script mutants at the pins, after one dry-run test was strengthened for two first-run survivors; 28 of 29 at `escape_scan.py`, after its fixture was strengthened for five survivors and a redundant guard removed for a sixth. The logs are in the stage agents' scratch.
- **Whole workspace** at `995e100`, once: **768 tests over 44 result lines, 767 passed, one ignored, none failed**, and no load flake fired. At the verification round's `ceb06ad`, once, through the machine's two-slot wrapper: **774 tests over 44 result lines, 773 passed, one ignored, none failed**, and no load flake fired; 511 in `cbr-provider` and 196 in `cbr-cli`.
- **Gates at the head**: `check_docs.py` 0 errors; `verify_pin.py` ok; `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings` and `cargo build --workspace --locked` clean; `git diff --check origin/main...HEAD` clean; no machine path in any tracked file. The gates above were run again at the verification round's head, all clean, with `check_docs.py` over 32 Markdown files. The seven standard suites, each into its own temporary directory and each matching its expectation, run with the code of `995e100`, and were not run again for the round, whose code changes a launch only in the list its dialect name is read through, not in any name it accepts, and in a named constant for the same 8 rows: stream 24 pass; core 130 pass, 5 unsupported; socket 11 pass, 2 unsupported; evidence 16; knowledge 10; context 11; composition 3 pass, 11 unsupported. `result_paths.py conformance/results/*/`: nine directories, no machine path.

**Verification round, 2026-09-26.** Two independent verifiers checked `f3d203b`, one the arithmetic and one the cuts, each with its own mutants; `dc2cedf` is its tests first, with stubs, observed red (nine `cbr-provider` tests and the control-character scan), and `b2f9a77` its fixes.

- **Should fix: the dialect.** The pins priced only `Dialect::Responses`, and a serving launch may configure `openai` or `anthropic`, whose bodies are wider. Every published figure is now the most over `wire::Dialect::ALL`, through one helper, `model::over_every_dialect`, and `Dialect::parse` reads a configured name through that list and nothing else, so a new dialect is configurable and priced or neither. The OpenAI dialect frames the most on every body, and every figure rose: the flow by 1,136 (517,492 to 518,628), a selection question by 446 (167,307 to 167,753), a projection by 4,792 (336,424 to 341,216). The verifier's probe gave the same figures. Tests: the pins; `wire::tests::every_dialect_a_launch_can_name_is_one_the_published_figures_price`; `admission_reserves_exactly_what_send_worst_prices` and the ceiling tests' `every_send`, now over every dialect. Mutants killed: D1 (`ALL` names Responses alone), D2 (`ALL` without OpenAI while `parse` keeps its own list), D3 (the least over the dialects), D4 (the first dialect only) and D5 (`send_worst` frames every body as Responses).
- **Should fix: selection's rows.** The figure assumed 8 candidates, copied from a literal at the call site with nothing tying them. `selection::CANDIDATES` is now both. Tests: `selection::tests::the_call_site_asks_retrieval_for_the_candidates_the_figure_is_priced_for`, a source guard, and `model_selection::a_selection_offers_no_more_candidates_than_its_figure_is_priced_for`, where a file of fourteen matching sections is offered `c1` to `c8`. Killed: X1 (`rows: 9`, the verifier's surviving X2), X2 (the constant plus one) and X3 (the constant at 9, which moves the pins).
- **Notes.** The repair-body test compares every repair with a body built by hand, and kills R5 (NoToolCall's repair resent unchanged, the verifier's survivor) and R7 (every prose repair resent unchanged). `a_count_above_the_completions_bound_settles_at_that_bound` kills C1 (a count settled uncapped) and C3 (capped at its own reservation) without the sweep's grid point. The m4e stop test asserts that both refusals state `flow + 2 × selection` and "2 selection questions", and kills S6 and S7, the verifier's survivors. `escape_scan.py` escapes DEL and C1 in a path as well as C0, and `the_scan_never_prints_a_control_character_of_a_path_raw` kills E9 to E11. The freed-room window is described as the code has it, from the local refusal through the count's admission, send and settlement to the completion's admission, and `room_freed_anywhere_from_the_refusal_to_the_completions_admission_adds_less_than_the_count` frees the room at each place the call path names with every honest count, pinning an attempt's most at `W + Ic − (local − Ic)`; it kills W1 (a completion on a count reserving the local bound) and the harness mutant H7. The escape-dense journey checks every step's send against its own figure, and its `charged` check says it is a floor that binds on a real model.
- **Mutants**, each applied alone in a private archive copy and counted killed only when a test that passed at the fix head failed: 18 of 18 production and script mutants, and 1 of 3 harness mutants. A6 (NoToolCall dropped from the widest repair) and A7 (the depth written as 1) survive as equivalent: NoToolCall's repair is NotStructured's body, and `REPAIRS` is 1. H3 (the boundary sweep's run with no ceiling dropped) and H4 (the ten-times-local count dropped) survive as redundant grid points. One run of H4 failed ten keychain tests on the fake tool's `TimedOut`, a load flake, not counted; re-run alone, it survived.

**Survivors and things recorded, not fixed.** A truncation-only `question_worst` is not caught by the published pins, only by the synthetic bodies, the sweep and the freed-room test, because truncation is the wider repair on every published body. Which of two equally wide spans `escape_scan.py` names widest is not tested. `config.rs` still defaults the `model.fake` control to the literal `Counting::WhenItCouldAdmit`, which the source test does not cover, and that variant keeps a stale comment. A run's m4e price can pass the per-job ceiling, at three wants, and it is the harness's stop and not a bound on a job. The J2 rubrics quote 492,204 and are frozen texts with recorded digests, so their notes are in JOURNEYS, not in them.

**For m5b, when it rebases onto this** (plan §10; m5b/loop is `57c0412` locally, which contains the `a0f1865` the plan read):

1. Take `main`'s `model::question_worst`, which has m5b's signature, so `loop_ops.rs:755` compiles unchanged, and drop m5b's own four-send body and its documentation.
2. In `investigation/tests.rs`, delete the local `sent`, `counted`, `repaired`, `question_worst`, `discovery_worst` and `selection_worst`, and use `crate::model::{send_worst, repaired, question_worst}`, `model::harness::boundary_sweep`, `discovery::tests::PUBLISHED_FLOW` and `selection::tests::PUBLISHED_SELECTION_QUESTION`.
3. Move its turn body to the longest model, and re-pin the turn and the loop as the most over every dialect (`model::over_every_dialect`), and its room, which becomes 313,619. The turn and loop figures are m5b's to compute: the earlier estimates here were the Responses dialect's. Keep `LOOP_MAX_CALLS = 16`: four sends a question is still a true count of sends.
4. Take `main`'s side for `discovery/tests.rs`, m4 READINESS §3 (dropping 489,192), `m4e_run.py` and m5 READINESS §8, delete `discoverys_published_flow_covers_a_count_before_every_attempt_…`, and stop quoting 489,192.
5. The guard comment at `loop_ops.rs:755`, "the ledger never refuses a turn the loop asked", holds only when no concurrent question reserves in between; the ledger still enforces the ceiling.

**Ruled on by the owner, 2026-09-26** (plan §12, now settled in [m5 READINESS](m5/READINESS.md) items 7-18):

1. **The cut**, 8,192 and 1,024 carried bytes, as [ADR 001 question 17](../decisions/001-standalone-v0.1-scope-and-stack.md), **accepted** (item 12).
2. **Resolved 2026-09-26: the owner approved m5-settle, built above (item 11).** ~~**m5-settle**, and when: the admission and settlement changes §1 reserves to the owner ([m5 READINESS §9](m5/READINESS.md#9-the-pull-requests-each-with-its-gate)). The recommendation is before m5h's live loop run, the first run likely to come near a job's ceiling.~~
3. **J2's harness bound**: **kept at 492,204** (this session's decision, at or above the projection worst case of 341,216).
4. **The order**: **m5-arith goes before m5b** (item 13).
5. **`Counting::Always` for m5b**: **the loop never runs under `Counting::Always`** (this session's decision, for the owner to overturn).
6. **m5e**: a projection, the flow and a selection question in one job is 1,027,597, over the job's ceiling, and m5e has to size it. Still open; not among the owner's 2026-09-26 rulings.

**Next.** A re-verification of the round's fixes, then a pull request, which this stage does not open. m5b rebases onto this only once it has merged.

## Earlier — packet_invalid

On `compiler/packet-invalid`, off `main` at `651c86e` (#46, J2's rerun record). Commits: `df9a709` tests first, `104615d` the implementation, `07bc1ea` a test for N13b, which the mutant run showed was missing, and `45d9268` the docs; then, after independent verification, `f9980b3` six tests for what its mutants and its claims check reached ([below](#after-verification)), and `fb39ca1` corrections to the docs; then, after a second independent verification, `6e3f838` three tests for what its mutants reached ([below](#after-the-second-verification)), `448b3c2` a corrected doc comment on `end_refused_job`, and `a21a7ae` corrections to the docs; then, after a third, `fa6b3bf` four tests for what its mutants and probes reached, with corrected comments on T1 and T3 ([below](#after-the-third-verification)), and `7119cd5` corrections to the docs; then, in a fourth fix round, on a defect the third verification's probe showed and the orchestrator's decision, `eeb0a55` tests first, `5891542` the fix, `a98235c` two tests for the other call sites of `publish_one`, and `57e6e1d` the docs ([below](#fix-round-4)); then, after a fourth verification, which found the code correct, `6deafa9` five tests and three extended for what its mutants and its claims check reached, and `48104d4` the docs ([below](#after-the-fourth-verification)); then, after a fifth verification, `2819598`, which bounds two polling loops in the tests, and these docs ([below](#after-the-fifth-verification)). **No model has been called.**

- **The ruling.** The owner's ruling of 2026-09-26, question 12 of the session's list ([m5 READINESS](m5/READINESS.md) item 7): *"#43's leftover question: a packet the id guard refuses should end its job with a typed `packet_invalid`. I recommend yes."*, answered *"10-12 I agree with your recommendation"*. It closes the open policy question the packet ids change left ([below](#earlier--packet-ids-within-the-identifier-grammar)). The ruling names the job's ending and its reason; **that the request ends `refused` is this session's reading within it**, recorded as such in [VERIFICATION](../VERIFICATION.md#the-deterministic-packet-compiler-and-what-a-packet-is-compiled-from).
- **The protocol: the reason set is open and the state set closed, so nothing in it changes.** The inspect result schema types `reason` as `{"type":"string","minLength":1,"maxLength":64}` with no enum (`schemas/context/1/context.request.inspect.result.schema.json`, and the submit result the same); `common.schema.json`'s `item_result` has `result` in `satisfied`, `unmet`, `degraded`, `pending` and the same `reason`. CONTEXT §3 lists item reasons ending "or a provider reason", and §4 says "Ending the job itself is a provider decision reported as `context.job.ended` with its reason". The request state is the closed enum `preparing`, `ready`, `partial`, `unmet`, `refused`, `cancelled`, and none is added. `ready`, `partial` and `unmet` each need a published revision (§5: "When a request reaches `ready`, `partial` or `unmet`, the provider publishes a packet revision"), `cancelled` is the caller's, and `preparing` is what the owner ruled out; `refused` is terminal (§4: cancelling one is `not_found`). **The risk, recorded:** §3's "After preparation the request is `ready` … `partial` … `unmet`" can be read as requiring one of those three; this change reads preparation as never having produced a revision that could be published, which §5 and §12 ("Until the artifact is sealed, the revision is not published and the request stays `preparing`") tie those states to.
- **What the provider does.** When the guard refuses a packet, the tick's batch is dropped as before, so nothing of the refused tick is committed. **Nor does that tick seal any packet, anywhere** (since [fix round 4](#fix-round-4)), though an evidence peer may already hold one an earlier tick sent it (choice (d) below): a packet that passes the guard is held in the tick, and the tick's packets are captured, sent to an evidence peer or written as objects only once the tick has run to the end, every packet it publishes guarded, and before its batch commits (`seal_packets`). `end_refused_job` (`crates/cbr-provider/src/provider/context_ops.rs`) then reads the job and its requests **as stored** and, in one provider batch of its own: every subscriber still `preparing` is `refused` with reason `packet_invalid`, its items ended by `context::refused_items` — `unmet` if required, `degraded` if advisory, for that reason, read off the request alone and never off the job's sections — with `context.request.changed {state: refused, job, reason}`; a subscriber already published under `context.updates` keeps its state and current revision; the job is `ended` with reason `packet_invalid` and `context.job.ended {reason: packet_invalid}` is recorded after the requests' changes. The barrier `context.packet.refused_before_commit`, declared in no descriptor, sits before the one commit; the job's answers are released after it, as a cancel does. **The log line is written after that commit**, word for word as before plus `; the job has ended: packet_invalid`, and still names no id. `context.request.inspect` serves the ended items when there is no published revision. No packet byte changes: the golden digest, `ids.rs`, the compiler, the CLI, every fixture and `Cargo.lock` are as they were.
- **Resulting shape.** `state: "refused"`, `reason: "packet_invalid"`, no `needed`, `packets: []`, `job: {kind: "context.job", id}`, each required item `unmet` and each advisory item `degraded` with reason `packet_invalid`, the revision raised once. Never produced by the refused tick, for the refused packet or for any other packet it publishes, a valid one included: a capture, a send to the evidence peer, an object, an artifact, a seal or `context.packet.published`. A valid packet of the same tick, for another subscriber of the job or an earlier revision, is dropped with that tick, which seals it nowhere: choices (b) and (d) below. **That is not "never sealed at a peer"**: a revision whose send to an evidence peer timed out in an earlier tick may since have been sealed by the peer, and the next tick's refusal then ends the request `refused` beside it. The guard cannot undo a peer's seal (choice (d), follow-up 8).

### Red, then green

The tests were written first (`df9a709`) by an agent that did not write the implementation, and observed red there; the provider binary at `df9a709` behaves as `651c86e`'s, because the scaffold `refused_items` it adds is dead code outside the unit tests. At `651c86e` itself the unit test does not compile (`error[E0425]: cannot find function refused_items`).

| | Test | Red at `df9a709` |
|---|---|---|
| E1 | `a_packet_holding_an_id_outside_the_identifier_grammar_is_never_published` (edited) | `tests/context.rs:1101`: `left: "preparing"`, right `"refused"` |
| E2 | `a_scripted_packet_missing_a_required_id_is_never_published` (edited) | `:1266`: `r-no-section-id: … left: "preparing"` |
| T1 | `a_packet_the_guard_refuses_ends_its_job_and_refuses_its_request_as_packet_invalid` | `r-bad never left preparing within 50 polls` |
| T2 | `a_refused_request_stays_refused_across_ticks_and_restarts_and_its_job_is_never_compiled_again` | `r-bad never left preparing within 50 polls` |
| T3 | `every_request_sharing_a_job_the_guard_refuses_is_refused_with_it` | `r-first never left preparing within 50 polls` |
| T4 | `an_update_the_guard_refuses_ends_the_job_and_leaves_the_published_revision_current` | `:1735`: `the job never ended: a refused update left it running` |
| T5 | `a_refusal_commits_nothing_of_the_tick_that_was_refused` | `r never left preparing within 50 polls` |
| T6 | `a_request_refused_after_content_was_prepared_reports_no_item_satisfied` | `r-bad never left preparing within 50 polls`, with `i-1` `satisfied` while preparing, so its premise holds |
| T7 | `a_kill_before_the_refusal_commits_leaves_nothing_and_the_restart_refuses_once` | `:315`: `the refusal's batch reaching its commit never happened` (20 s) |
| T8 | `a_request_the_guard_refuses_at_its_deadline_ends_refused_too` | `r-bad never left preparing within 50 polls` |
| U1 | `context::tests::a_request_refused_after_preparing_satisfies_no_item` | `left: []` against the three ended items |
| J | `a_request_whose_packet_would_carry_an_id_outside_the_grammar_ends_refused_and_says_why_through_cbr` (`expand.rs`) | `:1481`: `bad never left preparing` (60 s) |
| T9 | `a_refusal_holds_back_no_other_job_the_same_tick_walks` (`07bc1ea`, after the mutant run) | Written after the implementation, so its red is its mutants: N13b, N13a and M18 at `:2019`, *the refusal held back the job the tick walked after it*; N1, the arm as at `651c86e`, at `:2030`, `r-bad` not refused |

The whole workspace at `df9a709`, under `wslot`: 757 tests over 43 result lines, 744 passed, one ignored, 12 failed, and the 12 exactly these. Mutant N1 below, the arm only logging as at `651c86e`, reproduced every red in the table but U1's and T9's at the same lines, against the implementation's head; U1's red is N12a and N12b, T9's is above. Green at `104615d`: `tests/context.rs` 18 of 18, `context::tests` 17 of 17, `expand.rs` 15 of 15; at `07bc1ea`, `tests/context.rs` 19 of 19; at `f9980b3`, 25 of 25; at `6e3f838`, 28 of 28; at `fa6b3bf`, 32 of 32; at `5891542`, 35 of 35; at `a98235c`, 37 of 37; at `6deafa9`, 42 of 42; at `2819598`, 42 of 42.

### The mutant table

**31 mutants, 30 killed; the one survivor is the equivalent the plan expected.** Each was applied to `104615d`'s sources by a script, the workspace built, and `tests/context.rs`, the provider's `context::` unit tests and J run; then the sources were restored. A kill counts only when it names a test listed at the head. The two survivors of that run, N13b and N15, were then run against every `cbr-provider` test binary (503 tests over 8 result lines) and the whole of `expand.rs`. The script and its logs are in this session's scratch. #43's guard mutants M14–M18 were re-run too.

| Mutant | Killed by |
|---|---|
| N1 the refusal arm only logs, as at `651c86e` | E1, E2, T1–T8 and J |
| N2 the request ends `unmet` | T1, J and 8 more |
| N3a `PACKET_INVALID` spelled `unavailable` | T1, T4, J and 7 more |
| N3b the job's reason alone `deadline_passed` | T1, T3, T4, T8 |
| N3c the items' reason alone `unavailable` | T1, T3, T6, T8, J |
| N3d the request record's reason alone `deadline_passed` | T1, J and 7 more |
| N3e the `request.changed` event's reason alone `unavailable` | T1 |
| N4 the requests refused and the job left `running` | T1, by the stored job. The plan also named T2, which passes: a refused request is no longer due a revision, so the guard is not reached again and nothing more is logged or ended |
| N5 no `context.job.ended` | T1, T2, T3, T4, T7, T8 |
| N6a no `context.request.changed` | T1, T7 |
| N6b the change without its `reason` | T1 |
| N6c the change without its `job` | T1 |
| N7 only the first subscriber refused | T3 (`r-second never left preparing`) |
| N8 every subscriber refused, whatever its state | T4 (`a request that was delivered a packet was refused`) |
| N9 the refused tick's batch committed as well | T5. The plan also named T1's job row, which passes: there the refused tick had written nothing before the guard |
| N10 the tick's advanced job saved instead of the stored one | T1 (`cursor 0`, `sections []`) |
| N11 inspect ignores `ended_items` | T6 (`satisfied`), T1 (`pending`), T3, T8, J |
| N12a `refused_items` swaps `unmet` and `degraded` | U1, T1, T3, T6, T8, J |
| N12b `refused_items` leaves a required item `pending` | U1, T1, T3, T6, T8, J |
| N13a the arm returns `Err` and never ends the job | E1 and E2 (`a valid job beside the refused one never published`), T1–T8 and J |
| N13b the job ended and logged, then the tick stopped with `Err` | **Survived the first run**: the refused job now ends at its first refusal, which in E1 comes before the valid job exists. Run again with T9 added, against every `cbr-provider` binary and the whole of `expand.rs`, only T9 failed. **Killed by T9**, `a_refusal_holds_back_no_other_job_the_same_tick_walks` (`07bc1ea`), at `:2019`, *the refusal held back the job the tick walked after it*, `left: "preparing"`; T9 also kills N13a, M18 and N1 |
| N14 the log suffix names the refused section ids | E1 (`the log repeats the id itself`) |
| N15 `release_job` dropped | **Survives, equivalent as expected**: 503 of 503 `cbr-provider` tests and 15 of 15 in `expand.rs` pass. A compiled job's compile frees its answers before the guard runs, and a scripted job has none |
| N16 the log line written before the ending commits | T7 (`a refusal was logged before it committed`) |
| N17 the ending committed in two batches, requests then job | T7 (the stored request `refused` after the kill, `preparing` expected) |
| N18 the ended items read off the stored job's sections | T6 |
| M14 (#43) the guard removed | E1, E2, T1–T8 and J |
| M15 (#43) the guard checks sections only | E2 and three guard unit tests: `the_guard_names_every_id_outside_the_grammar_by_where_it_is`, `the_guard_reads_the_sealed_body_where_it_differs_from_the_facts`, `the_guard_names_a_required_id_that_is_absent_or_null` |
| M16 (#43) the guard skips the uniqueness check | `the_guard_refuses_one_section_id_or_one_citation_id_named_twice` |
| M17 (#43) the guard runs after the seal | E1, T2 and T5, each by an object count: the refused packet's object is written |
| M18 (#43) the guard's failure routed through `Protocol` | E1, E2, T1–T8 and J; and T9 |

### After verification

Two agents that wrote none of this verified it at `45d9268`. The code verifier's mutants V10, V13, V16, V18 and V57 survived every `cbr-provider` test, and the claims verifier found that T3's comment, *"whatever its own deadline"*, claimed what no test pinned. Each probe they left uncommitted is now a test in `tests/context.rs` (`f9980b3`), passing at the head. Each kill below was shown by applying the mutant by hand to the provider's sources, running `tests/context.rs` and restoring the sources byte for byte; only the named tests failed. No provider source changed. The line numbers are `tests/context.rs`'s at `f9980b3`; later commits moved them.

| Test | Mutant, and its red |
|---|---|
| T10 `a_shared_job_whose_first_subscriber_was_cancelled_still_ends_for_the_one_left` | V10, the job looked up by the refused request's id (`end_refused_job(&request)`): `r-second never left preparing within 50 polls`. V13 too, at `:2119` |
| T11 `every_refusal_in_a_shared_job_names_the_job_and_not_the_request` | V13, `context.request.changed` naming a job whose id is the request's: `:2163`, left job `r-second`, right `r-first` |
| T12 `a_subscriber_with_a_later_deadline_is_refused_with_the_job_an_earlier_deadline_ended` | No verifier mutant was aimed at it. VL, this round's own, refuses only the subscribers whose deadline is the job's earliest: `r-second never left preparing within 50 polls`, and T3, whose two deadlines are equal, passes under it. V13 too, at `:2231` |
| T13 `a_request_submitted_after_the_refusal_never_joins_the_ended_job` | V57, `may_join` without the running check: `:2271`, left job `r-bad`, right `r-later` |
| T14 `a_request_published_unmet_or_partial_keeps_its_state_and_revision_when_a_later_update_is_refused` | V18, only a `ready` subscriber skipped: `:2362`, *a request that was delivered a packet was refused*. Revision 1 is `unmet` for one request and `partial` for the other |
| T15 `the_log_line_of_a_refused_packet_says_its_job_has_ended_and_names_no_id` | V16, the log suffix's condition inverted: `:2405`, the line without *; the job has ended: packet_invalid*. The line is pinned whole, and names no id |

T3's comment now claims only what it pins, and names T12 for the later deadline. The comments in E1 and J credit the owner with the job's ending and this session's reading with `refused`, as the rest of this section does.

### After the second verification

Two more agents that wrote none of this verified it at `fb39ca1`. The code verifier's mutants W1, W2, W3, W6 and W15 survived every `cbr-provider` and `cbr-cli` test in its run; each is now killed by a test in `tests/context.rs` (`6e3f838`), passing at the head. **No mutant was applied in the worktree.** Each kill was shown in a private copy of the tree, `git archive fb39ca1` with those tests applied: the mutant applied by script, the workspace built, `tests/context.rs`, the provider's `context::` unit tests and `expand.rs` run, and the copy restored. Each time only the named test failed; the unit tests (17 of 17) and `expand.rs` (15 of 15) passed. The line numbers are `tests/context.rs`'s at `a21a7ae`, where each is the line of the failing assertion; at `6e3f838`, the commit that added the tests, each was one line earlier. *(Until the fourth verification this sentence said "at this head", which later commits made untrue.)*

| Test | Mutant, and its red |
|---|---|
| T16 `a_refused_request_reads_each_of_its_items_ended_in_the_order_it_submitted_them` | `i-1` required before start, `opt` advisory and `t-1` required before a transition, read through `context.request.inspect` as `unmet`, `degraded` and `unmet`, each for `packet_invalid`. At `:2510`: W1, `refused_items` testing for `required_before_start` alone, `t-1` `degraded`; W2, the inspect dropping advisory entries, `opt` missing; W15, `ended_items` reversed, `t-1` first. The submitted order is also the ids' alphabetical order, so T16 tells it from its reverse and not from a sort, as its comment says |
| T17 `a_refusal_is_recorded_at_the_provider_clock_of_the_tick_that_refused` | Submitted at 00:00 and refused at 00:10, on a clock file that moves. W3, the refusal's batch at a fixed instant: `:2560`, the refusal and `context.job.ended` recorded at `2000-01-01T00:00:00Z`, the submission at 00:00 |
| T18 `an_ended_job_is_still_seen_through_a_subscriber_it_is_not_named_after` | Job `r-first`, shared with `r-second`, refused; the session issues itself a grant of `core.events.read` and `context.read` over `context.request` `r-second` alone. `core.events.read` under it shows nothing of request `r-first`, and shows job `r-first`'s `context.job.ended`, which it can see only through the ended job's subscriber list (`context_visible`). W6, the ended job's `requests` cleared: `:2651`, `left: []` |

The same round found `end_refused_job`'s doc comment claiming more than is true: *"nothing of the refused packet is kept anywhere"*. A probe in which one tick commits the bad section and a later one, at the deadline, publishes it found the section in the ended job's stored record. That record is never served, and nothing of it is logged; the comment now says exactly that (`448b3c2`). This docs commit corrects the rest of what the round found: J4's control, worded in READINESS §7 and §9 and in JOURNEYS as the owner decided (READINESS item 9); item 9's quote, labelled as question 2's recommendation; the line numbers above, cited at `f9980b3`; notes on the m5g and J2 rerun sections below, which were written before the owner's rulings; and three comments, in J, T1 and the compiler section of VERIFICATION, that called `refused` the one terminal state needing no packet, when `cancelled` needs none either and is the caller's. (*This paragraph and `a21a7ae`'s message named E1; `a21a7ae`'s diff edits T1, `a_packet_the_guard_refuses_ends_its_job_and_refuses_its_request_as_packet_invalid`. Corrected after the third verification, which also found T1's comment citing CONTEXT §3 for what §5 says, at line 69 of the vendored profile: "When a request reaches `ready`, `partial` or `unmet`, the provider publishes a packet revision". T1 now cites §5 (`fa6b3bf`).*)

### After the third verification

Two more agents that wrote none of this verified it at `a21a7ae`. What they found:

- **Two claims were untrue.** Choice (b) and T3's comment said that no narrower subscriber's own packet could be valid, because the guard reads omission ids. It reads an omitted section's id only; the citation and claim ids inside that section never reach the narrower subscriber's packet. Both verifiers' probes showed a narrower subscriber that is published alone refused with a shared job. And W4 was recorded below as out of reach without fault injection. It is not: holding SQLite's write lock from a second connection while the ending waits at its barrier makes the commit fail, and under W4 the refusal is logged although it never committed.
- **Two mutants survived** every `cbr-provider` and `cbr-cli` test in the code verifier's run: Z5, the ended job's `requests` pruned to the subscribers it refused, and Z17, the job's ending recorded after the first subscriber's refusal and before the rest.
- **Three wording faults**: T1 cited CONTEXT §3 for what §5 says; the paragraph above named E1 for T1; and m5 READINESS's decisions preamble called every item but item 18 a recommendation, when questions 5 to 9 had been put as what this session would do unless the owner said no, and were answered *"sound good"*.

**The behaviour is kept**: a packet the guard refuses still ends the whole job. That is this session's decision, and choice (b) below gives its reason; the owner is told it as information and may overturn it. `fa6b3bf` adds four tests, T19–T22 in `tests/context.rs`, each passing at the head, and corrects the comments on T1 and T3; this docs commit corrects the rest. **No provider source changed.** Each kill below was shown in a private copy, `git archive fa6b3bf` with an `origin` remote added for `j2_harness`: the mutant applied by script, the workspace built, every `cbr-provider` and `cbr-cli` test binary run (28 result lines), and the copy restored. At the head that run gave 703 passed and one ignored; under each mutant only the named tests failed. **No mutant was applied in the worktree.** The line numbers are `tests/context.rs`'s at `fa6b3bf`.

| Test | Mutant, and its red |
|---|---|
| T19 `a_narrower_subscriber_whose_own_packet_is_valid_is_refused_with_its_job` | `r-narrow`, at 64 bytes of output capacity, omits `s-more` for capacity, and `s-more`'s one citation id is outside the grammar. Alone, as the control, it is published `ready`, its revision naming `s-more` only as an omission. With `r-wide`, at 4096, in its job, both are refused as `packet_invalid`; nothing is published, served or committed as an artifact; the one object left is `r-narrow`'s packet, the very bytes it is published as alone, and the next start collects it. P1, this round's own, a refusal per subscriber — each preparing subscriber's own packet compiled from the refused tick's job and put to the guard, and only those whose packet fails refused: `:1397`, *r-narrow never left preparing within 50 polls*. It fails no other test, T3 included, where both subscribers' own packets fail |
| T20 `a_store_failure_while_ending_is_a_tick_failure_and_the_next_tick_ends_the_job_once` | The ending held at `context.packet.refused_before_commit` while a second connection holds `BEGIN IMMEDIATE` past the provider's 10-second busy timeout, then released: the tick fails (*context preparation failed*), no refusal is logged, the request is still `preparing` and the job `running`; the next tick ends the job, with one log line carrying the suffix, one refusal and one `context.job.ended`. W4, the store error swallowed (`.unwrap_or(false)` for `?`): `:2948`, *a refusal was logged although its ending never committed*, `left: 1` |
| T21 `a_published_subscriber_not_named_by_its_job_still_sees_the_ending` | Under `context.updates`, `r-first` and `r-second` share job `r-first` and are both published `ready`; the update is refused, the job ends, and neither is refused. A reader whose grant covers `r-second` alone still reads the job's `context.job.ended`. Z5: `:3069`, `left: []`. W6 fails it too, beside T18 |
| T22 `every_subscribers_refusal_is_recorded_before_the_jobs_ending` | Both refusals of a shared job precede `context.job.ended` in the stream. Z17: `:3109`, *r-second's refusal is recorded after the job's ending* |

**What is left of a narrower subscriber's valid packet**, confirmed from the code and two probes in this session's scratch rather than from a test: `publish_one` seals each subscriber's packet in turn, so one compiled before the refused one is already sealed when the guard refuses. Under the provider's own seal its object is written and no row names it, so the start-time collection pass removes it at the next start (T19). Under an evidence peer the packet has already been sent there: a probe with T19's two requests and `evidence_provider` set found `packet.r-narrow.1` `sealed` at the peer, no `packet.r-wide.1`, and no object or artifact in CBR's store. Choice (b) says so. *(Fixed in [round 4](#fix-round-4): no packet of a refused tick is sealed anywhere, and T19 now requires no object before the restart.)*

**Found, and not pinned here: Z22 is not equivalent.** The code verifier's Z22, the ended job's committed sections cleared, survived every test in its run and survives every test at this head (below). A probe shows it is observable: a subscriber cancelled before the ending still reads its items off the job's stored sections, as a cancelled request always has, so under Z22 its `i-1` turns from `satisfied` to `pending` once the job ends; at the head it reads the same before and after. The last bullet of *Recorded, not changed* is corrected accordingly. Pinning it was not among this round's four tests, and is left open. *(Pinned in [round 4](#fix-round-4), by T25.)*

### Fix round 4

**The defect**, which the third verification's probe showed ([above](#after-the-third-verification)): `publish_one` sealed each packet as soon as it had compiled and guarded it, before the guard had seen the next packet of the tick. So a narrower subscriber's valid packet, published before a wider one the guard refused, was already sealed when the job ended: at an evidence peer, `packet.r-narrow.1` `sealed` there although its request ends `refused` and nothing in CBR names it; under the provider's own seal, an object no row names, collected at the next start. T5's script does the same with one request: revision 1 sealed, then revision 2 refused, in one tick.

**The decision, the orchestrator's, as a defect fix**: the guard sees every packet a tick would publish before anything of any of them is captured, sent to a peer or sealed. If any is refused, nothing of the tick leaves the provider and the job ends `packet_invalid` as before; if none is, publication proceeds as before. The narrower subscriber is still refused with the job: choice (b) below, which the owner may overturn.

**The fix, `5891542`.** A packet that passes the guard is held in the tick (`Tick::outgoing`), while its request's publication — and, under the provider's own seal, its artifact's `staged` and `sealed` rows — is built in the tick's batch exactly as before. `seal_packets` lets the tick's packets leave, in the order the tick published them, once `advance` has run to the end and before the batch commits: each one for a peer captured, sent and sealed there, each of the provider's own written as an object, with `context.packet.after_object_published` after it as before. **That is not the per-step pre-pass the decision sketched, and deliberately.** A tick can publish at more than one step — T5's script publishes, prepares a section and publishes again — and a later step's packet does not exist until the steps before it have run, so a pre-pass at each call site of `publish_one` would still seal revision 1 before revision 2 is compiled. M2 below is that shape: T19 and T23 pass under it, and T5 and T24 fail. Holding every packet to the end of the tick covers every call site by construction — a `publish` step, an update under `context.updates`, a job's ending through `finish` and `publish_all_finishing`, and the deadline — and T19, T24, T26 and T27 each hold one of them.

**What is unchanged for a tick the guard does not refuse**: the same packets in the same order, with the same capture instants and descriptors, the same batch and events, and each object written and verified before the batch naming it commits, so the crash-matrix row stands as it was. What moves is when a packet leaves: at the end of its tick's steps rather than at its own step. A peer that does not seal still fails the tick, keeping the capture instants of the packets attempted so far, the failed one included; a tick whose later step fails now fails before any send. The objects of the sources and derivations a compiled job seals while it compiles (`seal_source`, `seal_derivation`) are not packets and are written where they were: a refused tick still leaves them, once, for the start-time collection pass. The seven conformance suites were run again for this change (*Gates at the head*, below); `composition.packets-are-sealed-at-a-separate-evidence-provider` passes.

| Test | Red at `7119cd5` |
|---|---|
| T23 `a_narrower_subscribers_valid_packet_never_reaches_the_evidence_peer` | T19's shape under an evidence peer, with the same request alone published at the peer as the control: `packet.r-narrow.1` and `packet.r-wide.1` must be `not_found` there with no event about either, and CBR's store must hold no object, artifact or publication. `eeb0a55` against `7119cd5`'s provider, in the worktree: *packet.r-narrow.1 reached the evidence peer*, `left: None`, `right: Some("not_found")` |
| T24 `no_packet_of_a_refused_tick_that_publishes_twice_reaches_the_evidence_peer` | T5's script under a peer, with a valid job beside it sealed there as the premise: *packet.r.1 reached the evidence peer* |
| T19, tightened | No object before the restart, where it allowed `r-narrow`'s: *r-narrow's packet was sealed in the refused tick*, one object |
| T5 `a_refusal_commits_nothing_of_the_tick_that_was_refused`, tightened | No object before the restart, where it allowed revision 1's: *revision 1 was sealed in the tick that was refused*, one object |
| T25 `a_subscriber_cancelled_before_its_job_is_refused_still_reads_the_items_the_job_prepared` | Z22 pinned, in the probe's shape: `r-second`, cancelled while the job holds a committed section, reads `i-1` `satisfied`, and reads the same bytes after the job is refused. A pin, so green at `7119cd5`; Z22 fails it |
| T26 `a_narrower_subscribers_valid_packet_is_not_sealed_when_the_jobs_ending_refuses_the_wider_one` | T19's two subscribers with the script ending the job as `budget_insufficient`, as a compiled job that cannot fit does, so `finish` publishes both. Written after the fix (`a98235c`); red against the provider at `7119cd5` in the private copy (B0), `r-narrow`'s packet the one object. *(Since [the budget_insufficient fix](#this-change--a-compiled-request-that-cannot-fit-is-refused) its script ends the job as `investigation_budget_exhausted`: a compiled job that cannot fit no longer ends through `finish`.)* |
| T27 `a_narrower_subscribers_valid_packet_is_not_sealed_when_the_deadline_refuses_the_wider_one` | The same with the script stalling and the clock at both requests' deadline, which publishes both. As T26, red under B0 |

At `eeb0a55`, with `7119cd5`'s provider, `tests/context.rs` gave 31 passed and exactly T5, T19, T23 and T24 failed; at `5891542`, 35 of 35; at `a98235c`, 37 of 37. **The mutants ran in a private copy only**, `git archive 5891542` with `a98235c`'s two tests added and an `origin` remote for `j2_harness`: each applied by script, the workspace built, every `cbr-provider` and `cbr-cli` test binary run (28 result lines), and the copy restored. At its head that run gave 708 passed and one ignored; under each mutant only the named tests failed. **No mutant was applied in the worktree.**

| Mutant | Failed |
|---|---|
| B0 the provider as at `7119cd5` | T5, T19, T23, T24, T26, T27; T25 passes, as a pin does |
| M1 the old order: a packet leaves the tick as soon as it is guarded (`seal_packets` called in `publish_one`) — the mutant the decision named | the same six |
| M2 the per-step pre-pass: each call site lets its packets leave once it has guarded them all | T5 and T24 only |
| M2a the old order at the deadline alone | T27 |
| M2b the old order at a job's ending alone | T26 |
| M2c the old order at a `publish` step alone | T5, T19, T23, T24 |
| M3 the tick's packets leave after its batch commits | `a_publication_interrupted_at_the_evidence_provider_replays_after_the_clock_moves`: the first attempt reads `ready`, `preparing` expected |
| M4 the tick's packets never leave | E1 (no object for `r-good`), T23 and T24 (the premise's packet not at the peer), the crash row's test and the replay test (their barriers never reached) |
| M5 a peer packet's capture instant not kept | the replay test: the retry reads `preparing`, `ready` expected |
| Z22 the ended job's committed sections cleared (the third verification's) | T25: `i-1` `pending` after the ending, `satisfied` before |

### After the fourth verification

Two more agents that wrote none of this verified it at `57e6e1d`, and **found the code correct**: no defect of this change. What they found:

- **Six of the code verifier's mutants survived** every `cbr-provider` and `cbr-cli` test in its run: Q1, this provider's own packet objects written only after the batch naming them commits; Q2, `seal_packets` skipped in a tick that ends its job; Q3, `seal_packets` skipped in a tick that leaves the job's record unchanged; Q4, only the first of a tick's packets leaving; Q5, a peer send that fails keeping only the failed packet's capture instant; Q8, `end_refused_job` stopping (`break` for `continue`) at the first subscriber no longer `preparing`. Four more, Q6, Q7, Q14 and Q15, were killed at `57e6e1d`: Q6 (the failed packet's capture dropped), Q7 (`seal_packets`' failure ignored) and Q15 (a peer packet's reference naming this provider) by the replay test, Q15 also by T23, and Q14 (a store failure while ending skipped with `continue`) by T20.
- **One claim was untrue.** VERIFICATION said no packet of a refused tick is sent to an evidence peer whichever step published it, the job's ending and the deadline among them; T26 and T27, which hold those two steps, ran with no peer. The claims verifier's PX, a peer packet leaving as soon as it is guarded at a job's ending or the deadline, as before round 4, survived every test.
- **Five mutants are equivalent**, Q9–Q13, below.
- **A pre-existing defect**, which this change neither introduced nor fixes: a kill after an evidence peer's seal and before the batch commits leaves the request `preparing` for ever once the clock moves. It is follow-up 8 below, with the probe that showed it, and gets its own slice. *(Fixed since, by the peer-capture change above.)*
- **A wording fault**: *After the second verification* cited its line numbers "at this head"; they are `a21a7ae`'s, and it now says so.

`6deafa9` adds five tests, T28–T32 in `tests/context.rs`, and extends three: the crash row's test, T26 and T27. Each passes at the head. **No provider source changed.** The code verifier's Q5 probe had added a barrier after each peer send to the provider's source in its private copy; T31 needs none. It holds the evidence peer instead, at the peer's own barriers, and crashes and restarts it over the same store. Each kill below was shown in a private copy, `git archive 57e6e1d` with `6deafa9`'s tests applied and an `origin` remote for `j2_harness`. Each mutant was applied there from its diff: the code verifier's own diffs for Q1–Q5, Q8 and Q9–Q13, and the source hunk alone of the claims verifier's PX. The workspace was then built, every `cbr-provider` and `cbr-cli` test binary run (28 result lines), and the copy restored. At its head that run gave 713 passed and one ignored; under each mutant only the named tests failed. Under Q5, the first run also failed `idle_connections_keep_at_least_half_of_command_throughput`, VERIFICATION's named load flake, at a load average near 4 (K=0 9,497 commits, K=8 3,485); Q5 was run again alone, and only T31 failed. **No mutant was applied in the worktree.** The line numbers are `tests/context.rs`'s at `6deafa9`, each the line of the failing assertion.

| Test | Mutant, and its red |
|---|---|
| The crash row's test, `a_packet_is_never_published_before_its_local_seal_commits`, extended | At its barrier, before the kill, the store must hold the packet's one object, the request still `preparing` and no artifact row. Q1: `:558`, *the batch naming the object committed before the object was written*, `left: "ready"` |
| T28 `a_packet_published_at_its_jobs_ending_is_served_by_its_digest` | The script prepares `s-1` and ends the job, so `finish` publishes the request in the tick that ends it; the revision is fetched by its digest, and the job is `ended`. Q2: `:3696`, *r-1: served empty*: the fetch answers with no bytes, the object never written |
| T29 `a_packet_the_deadline_alone_publishes_is_served_by_its_digest` | The first tick commits `s-1` and stalls; the tick at the deadline publishes and leaves the job's stored record as it was, which the test checks as its premise. Q3: `:3696`, *r-1: served empty* |
| T30 `both_packets_one_tick_publishes_are_served_by_their_digests` | Two requests share a job whose one `publish` publishes both in one tick; both revisions are served. Q4: `:3696`, *r-second: served empty*. Q4 fails T31 too, at `:375`: *r-second's append at the peer never happened* |
| T31 `a_tick_whose_second_peer_send_fails_keeps_both_capture_instants_and_both_replay_after_the_clock_moves` | Two requests share a job whose `publish` at 00:10 sends both packets to a peer. First the peer holds `r-first`'s seal and CBR is killed there, so the peer holds `packet.r-first.1` sealed and CBR has kept nothing. Then the peer is restarted holding its first append, and CBR at the same instant: its tick replays `r-first`'s steps, which the peer answers from its record, and `r-second`'s append is held; the peer is crashed there. The tick fails on `r-second` with `r-first`'s send sealed in that same tick, and both instants must be kept. Last the peer is back and the clock at 00:15, and both requests must be `ready`, each sealed at the peer at 00:10. Q5: `:3886`, the kept captures `{"packet.r-second.1":"2030-01-01T00:10:00Z"}` alone |
| T32 `a_subscriber_published_at_its_own_deadline_does_not_stop_the_ending_for_one_still_preparing` | `r-early` and `r-late` share job `r-early`; `r-early` is published `ready` at its own deadline, 00:05, and the job runs on; at 00:10 the guard refuses the job's packet. `r-late`, listed after `r-early`, must be refused as `packet_invalid`, `r-early` read the same bytes, and the job be `ended`. Q8: `:1442`, *r-late never left preparing within 50 polls* |
| T26 and T27, extended | Each runs twice, in this store as before and beside an evidence peer, which must never hear of `packet.r-narrow.1` or `packet.r-wide.1`, with no send to it logged. PX: `:3336` in both, *packet.r-narrow.1 reached the evidence peer*, the peer holding it |

**The five equivalents, each run again with T28–T32 in place**, against every `cbr-provider` and `cbr-cli` test binary; each passed all 713, one ignored, as the head does:

- **Q9, the inspect's `ended_items` arm taken only for a `refused` request.** `ended_items` is written only by `end_refused_job`, in the same record write that sets the request `refused`, and `refused` is terminal: `publish_one` finds such a request due nothing, and a cancel of it is `not_found`. So every record with `ended_items` is `refused`.
- **Q10, a failed send's capture kept as the tick's instant rather than the one the send used.** `keep_captures` sets an artifact's instant only where the stored job has none, and the send used the stored instant when there was one and the tick's otherwise. So what is kept is the same.
- **Q11, `seal_packets` reading kept instants from the job as the tick loaded it rather than as it advanced.** Nothing in `advance` writes a job's `captures`; `keep_captures` is the only writer, outside any tick. So both jobs hold the same instants.
- **Q12, the guard run after a packet's seal is arranged rather than before.** Nothing arranged leaves the tick before `seal_packets`, and a refusal drops the tick's batch and its outgoing packets, so on every tested path the two orders are alike. **Not strictly equivalent**, though, in one case no test reaches: a refused packet whose artifact id already names an artifact in this store. There, `seal_locally`'s collision check would run first, and the tick would fail through `Protocol` instead of ending the job. That collision is follow-up 2's, and VERIFICATION lists it among the untested paths.
- **Q13, `end_refused_job` without its `running` check**, is V1 of *Recorded, not changed* below, equivalent for the reason given there.

### After the fifth verification

Two more agents that wrote none of this verified it at `48104d4`. **No provider source changed in this round**: `2819598` changes `tests/context.rs` alone, and these docs the rest. What they found:

- **Two polling loops in the tests had no bound.** The replay test, `a_publication_interrupted_at_the_evidence_provider_replays_after_the_clock_moves`, and T31 each polled a peer until a packet read `sealed`, so a seal that never completed would hang the test binary rather than fail it. Since `2819598` each stops after 20 seconds, as `wait_for` does, with a panic naming the packet and the last answer. Observed by changing each loop's awaited state to one that never comes, in the worktree after the commit and restored with `git checkout` straight afterwards: both tests failed at the new assertion, *packet.r-1.1 was never sealed at the evidence provider once its seal was released* and *packet.r-first.1 was never sealed at the peer once its seal was released*.
- **"Never sealed, here or at an evidence peer" was too strong** (probe P1): a revision whose send timed out in an earlier tick may since have been sealed by the peer, beside a request the next tick refuses. *Resulting shape* and choice (d) now say so, and follow-up 8 records it.
- **Follow-up 8's defect is reached without a kill** (probe P2): any failure of the tick's commit after the peer sealed leaves the request `preparing` for ever once the clock moves. Follow-up 8, VERIFICATION and m5 READINESS now say so.
- **Wording.** VERIFICATION's compiler section still counted twenty-seven tests holding the ending, leaving out the fourth verification's five; it listed event visibility of `context.job` subjects under a grant as untested, which T18 and T21 test; m5 READINESS item 11 gave m5-settle's timing and not its substance; follow-up 5 could be read as if protocol#18 were the issue it describes; and READINESS's decisions preamble did not say that a quoted question or recommendation is this session's. Each is corrected.
- **Mutants.** The code verifier ran twelve new mutants, K1–K12, and the fourth verification's Q1–Q5, Q8 and PX again, in its private copy (`git archive 48104d4`), each against every `cbr-provider` and `cbr-cli` test binary (28 result lines; at the head, 713 passed and one ignored). All but two failed named tests: K1, the tick's packets leaving in reverse order, T31; K2, the crash row's barrier before this provider's object is written, the crash row's test; K3, the refusal's barrier after its commit, T7 and T20; K5, a refused tick's valid packets sealed too, T5, T19, T23, T24, T26 and T27; K6, an ended item without its obligation, J, U1, T1, T3, T6, T8, T10, T12, T16, T19 and T20; K7, a refused request's revision raised twice, T1; K8, a subscriber still preparing under `context.updates` left `preparing` when its job ends, J, T5 and T24; K10, a failed send keeping no capture instant, the replay test and T31; K11, the ended job stored without its reason, T1 and T32; K12, the refusal's log line naming the job for the request, T10, T19, T23, T26 and T27; and Q1–Q5, Q8 and PX, each the tests the table above names for it. The two:
  - **K4, a peer packet's descriptor naming its artifact id as its source** (`Outgoing::Peer`'s `request`), **passed every cargo test and failed one conformance fixture**, `composition.packets-are-sealed-at-a-separate-evidence-provider`, at step 9: *result/descriptor/source/id: expected "r-1", found "packet.r-1.1"*; `context` stayed 11 of 11. **Recorded as held by that fixture and by no cargo test**, which VERIFICATION now says. No test is added for it in this round.
  - **K9, the inspect's ended arm taken only while there is no published revision** (`(None, Some(…))` for `(_, Some(…))`), passed every test. **Recorded as equivalent**, beside W14 (*Recorded, not changed*, below): the arm before it takes every record that has a revision and is not `preparing`, so the ended arm could see a revision only on a `preparing` request with `ended_items`, and `ended_items` is written only in the write that makes a request `refused`, which is terminal.

### After the sixth verification

**No provider source changed in this round.** `418993c` turns the sixth verification's two ready probes into committed tests, `a_refused_tick_sends_nothing_even_a_packet_an_earlier_tick_failed_to_seal` and `a_tick_that_fails_after_a_packet_writes_no_object_for_it`, and these docs correct three narrowed claims. **S3 and S1 are now tested**: applying S3 in a private copy fails the first at `assert_never_sent("packet.r.1")`; applying S1 fails the second at the no-new-object assertion. **S5 is harmless, and recorded as such**: it passed every test but one named load flake (the keychain fake tool's `TimedOut`), so it stays a survivor, not a defect.

### Choices for the owner to overturn, each pinned by a test

- (a) The request ends `refused`, not `unmet` with an invented packet: `a_packet_the_guard_refuses_ends_its_job_and_refuses_its_request_as_packet_invalid` and the journey.
- (b) **Every subscriber still `preparing` is refused, even one whose own packet would have been valid**: `every_request_sharing_a_job_the_guard_refuses_is_refused_with_it`, whose two subscribers share one deadline; `a_subscriber_with_a_later_deadline_is_refused_with_the_job_an_earlier_deadline_ended`, for one whose deadline is later than the one that brought the packet on; and `a_narrower_subscriber_whose_own_packet_is_valid_is_refused_with_its_job` (T19), for one whose own packet is valid. That case can happen: a section a narrower subscriber omits for capacity is named in its packet only by its section id, as an omission (`context::inclusion`), and the citation and claim ids inside it never reach that packet. *(Until the third verification this choice said such a subscriber practically could not exist, because the guard reads omission ids; it reads an omission's section id only.)* **Refusing the whole job is this session's choice, which the owner may overturn**; the alternative is a refusal per subscriber, which would publish each subscriber whose own packet passes the guard. The reason for the choice: since #43 the compiler builds every id inside the identifier grammar (`crate::ids`), so in production the guard fires only on a defect or on a scripted test control, and ending the whole job fails closed. What it costs: a subscriber's valid packet in the refused tick is never delivered, and the narrower subscriber is refused with the job. **No packet of a refused tick is sealed anywhere, locally or at an evidence peer** — the orchestrator's decision in [fix round 4](#fix-round-4), a defect fix and not one of these choices: not as an artifact or an object in this store (T19, and T26 and T27 for a job's ending and the deadline), and not at a peer (`a_narrower_subscribers_valid_packet_never_reaches_the_evidence_peer`, T23, and T26 and T27 again beside a peer since the [fourth verification](#after-the-fourth-verification)). *(Until fix round 4 it was sealed before the guard reached the wider subscriber's packet: under the provider's own seal as an object the next start collected, and under an evidence peer at the peer, with nothing in CBR naming it, which a probe showed.)*
- (c) The job's reason is `packet_invalid` even when the refused tick was also ending for budget or deadline; the earlier cause is lost.
- (d) A valid revision published in the same tick as the refusal is dropped with that tick, which seals it nowhere, here or at an evidence peer. Only a script can do this: `a_refusal_commits_nothing_of_the_tick_that_was_refused`, and under a peer `no_packet_of_a_refused_tick_that_publishes_twice_reaches_the_evidence_peer` (T24). So is another subscriber's valid packet in that tick, (b). **That is not "never sealed at a peer"**: a revision whose send timed out in an earlier tick may since have been sealed by the peer, and the next tick, replaying that revision and refusing the one after it, ends the request `refused` with `packets: []` while the peer holds the revision sealed. The guard cannot undo a peer's seal. Follow-up 8 records it, with the fifth verification's probe P1, which showed it. *(Until the fifth verification this choice said such a revision was never sealed, here or at an evidence peer, with no qualification.)*
- (e) Items read `unmet` or `degraded`, never `pending` or `satisfied`: `a_request_refused_after_content_was_prepared_reports_no_item_satisfied` and the unit test.

### Found, then fixed

**Fixed since, by [the budget_insufficient fix](#this-change--a-compiled-request-that-cannot-fit-is-refused)**; the finding as it stood: **a compiled request whose mandatory content cannot fit does not end as CONTEXT §3 says.** `compile` replaces the steps with `end budget_insufficient` (`context_ops.rs`, where it compares `mandatory_size` with the capacity), and `finish` then publishes every subscriber through `publish_all_finishing`: the request ends `unmet`, with a published packet and no `needed`. §3 says such a request "ends `refused` with reason `budget_insufficient` and `needed: { units: "bytes", amount }`". Submit-time `budget_insufficient` (`context_submit`) has §3's shape. The owner's to prioritise.

### Recorded, not changed

- **W4, a store error swallowed in `end_refused_job`'s caller** (`.unwrap_or(false)` for `?`), is killed by T20 since the third verification ([above](#after-the-third-verification)). Until then this bullet said no test could reach it without fault injection, which the provider does not have. None is needed: the barrier before the commit holds the ending, and a second connection holding SQLite's write lock past the provider's busy timeout makes the commit fail.
- **W5, W14, W16 and V1 survive as equivalent**, each for a reason in the code:
  - W5, the ended job also marked `published`: the only reader of a job's `published` is `may_join`, which already requires the job `running`, and no tick walks an ended job.
  - W14, the inspect's `ended_items` arm put before the published revision's: `ended_items` is written only on a request still `preparing`, which has no packet — a request becomes `preparing` only at submit, and publishing moves it to `ready`, `partial` or `unmet` — and that request is `refused` from then on, so no record matches both arms.
  - W16, the arm failing the tick when `end_refused_job` returns `Ok(false)`, and V1, `end_refused_job` without its `running` check: the arm is reached only for a job this tick loaded from the store as `running`, and nothing commits to that job's record between the load and the ending, the refused tick's batch being dropped. So the job is always found `running`, and `Ok(false)` never comes back.
- All five, W4 included, were run again after the second verification in the private copy with T16–T18 in place: each passed `tests/context.rs` (28 of 28), the provider's `context::` unit tests (17) and `expand.rs` (15). The second verification had run each against every `cbr-provider` and `cbr-cli` test binary. After the third, W5, W14, W16 and V1 were run again with T19–T22 in place, against every `cbr-provider` and `cbr-cli` test binary: each passed all 703, one ignored, as the head does. So did Z22.
- **An ended job's stored record keeps a section an earlier tick committed**, as the corrected doc comment says. No operation serves a job's record and the log names no id, so no section's content or id reaches a reader. *(Until the third verification this bullet said "nothing of it reaches a reader".)* One thing read off it does: a subscriber cancelled before the ending reads its item results off the job's sections, as a cancelled request did before this change, so Z22, which clears them, is observable ([above](#after-the-third-verification)), and T25 pins it since [fix round 4](#fix-round-4).

### Follow-ups, not fixed here

1. **Two refused shapes.** A request refused at submit (`budget_insufficient`) has no `job` and its items read `pending`, since inspect falls back to `item_results` over no job; a `packet_invalid` one has its `job` and items `unmet` or `degraded`. Both are schema-valid. *(Since [the budget_insufficient fix](#this-change--a-compiled-request-that-cannot-fit-is-refused), a `budget_insufficient` refusal decided after compiling has the second shape, with `needed`; the one decided at submit keeps the first.)*
2. **`seal_locally`'s artifact-id collision** still aborts the whole tick through `Protocol`, and so holds back every job after it in that tick.
3. **The ordinary arm of `tick_context` calls `release_job` before its commit**, where a cancel and this change release after.
4. **`cbr` has no events verb**, so the updates case — a `ready` request whose job has ended — shows through `cbr` only as nothing changing; its evidence is the provider test.
5. **An issue to the protocol, not filed**: CONTEXT §3 could say that `refused` may follow `preparing`, with a `job`. Filing it is the owner's call, and nothing about it has been filed. **It is not protocol#18.** [Combraton/protocol#18](https://github.com/Combraton/protocol/issues/18) is a separate issue, about the protocol's conformance runner, filed on this session's reading of the owner's reply to question 11 ([m5 READINESS](m5/READINESS.md) item 18); it has nothing to do with `packet_invalid`.
6. **`check_docs.py`'s slug rule drops every underscore**, as an inline marker, where GitHub keeps one inside a word: it computes `this-change--packetinvalid` for this section and GitHub `this-change--packet_invalid`. So no link here points at this section's heading.
7. **Z22 is pinned** since [fix round 4](#fix-round-4), by T25: a subscriber cancelled before a `packet_invalid` ending reads its items off the ended job's stored sections, and reads the same after the ending. *(Until then this item said no test held it.)*
8. **An evidence peer's seal cannot be rolled back, and two things follow from it.** Neither is this change's defect. The first is **fixed** by the peer-capture change (*Earlier — a peer's seal survives a lost commit*, above); the second is a limit of the guard, recorded here and in choice (d), and recorded there as the peer's state CBR cannot roll back.
   - **Fixed by the peer-capture change** (*Earlier — a peer's seal survives a lost commit*, above): **a tick whose batch fails to commit after an evidence peer has sealed its packet left the request `preparing` for ever once the clock moved.** A kill between the peer's seal and the commit did it, and so did any failure of the commit itself. Nothing had kept the capture instant, so the retry described the packet with a new `captured_at` and the peer refused it as `idempotency_conflict` at every tick. The fourth verification's code verifier showed it with a kill at a barrier after the peer's send (13 such log lines, the request still `preparing` past its deadline); the fifth's showed it with no kill, holding SQLite's write lock with `BEGIN IMMEDIATE` from a second connection while the tick sealed at the peer (probe P2: 15 such lines). Both probes are now tests. *(Until the peer-capture change this item recorded the defect as not fixed.)*
   - **A refusal can end a request beside a revision the peer has already sealed** (choice (d)). The fifth verification's code verifier showed it (probe P1, in its private copy): a tick at 00:00 publishes `r`'s revision 1 and sends it to the peer, which holds the seal past CBR's peer timeout, so the tick fails and keeps the capture instant (*packet packet.r.1 not sealed at the evidence provider (unreachable: …); retrying at a later tick*); the peer then completes the seal. At 00:10 the tick replays revision 1, runs on, and the guard refuses revision 2. The request ends `refused` as `packet_invalid` with `packets: []`, while the peer holds `packet.r.1` `sealed` with `captured_at` 00:00 and has no `packet.r.2`. Nothing CBR serves names `packet.r.1`; the ended job's stored `captures`, which no operation serves, still holds its instant. The refused tick holds back every packet it publishes, but it cannot undo what a peer sealed before it, and nothing here tries to.

### Gates at the head

At `418993c`, the last commit with tests, with this docs commit's edits in place; this docs commit changes Markdown and one test comment. Provider code changed in `5891542`, the fourth fix round's fix, and nowhere else since `104615d` but `448b3c2`'s doc comment; `6deafa9`, `2819598` and `418993c` change `tests/context.rs` alone.

- **Static gates, all exit 0:** `check_docs.py`, `verify_pin.py`, `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo build --workspace --locked`, `git diff --check origin/main...HEAD`, and `git merge-tree --write-tree origin/main HEAD`, which merges with no conflict.
- **`Cargo.lock`, `GOLDEN_PACKET_DIGEST` and `vendor/` are unchanged, and no machine path is committed** (`git grep` for home-directory and volume paths is empty).
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`, after `cargo build --workspace --locked`: 783 tests over 43 `test result` lines, 782 passed, one ignored (the stall harness) and none failed**; no load flake appeared. By crate: 20, 7, 40, 528 and 188; `tests/context.rs` 44 of 44 (the sixth verification's two tests added, `tests/context.rs` 42 of 42 at `2819598`). At `6deafa9`, with `48104d4`'s docs in place, the same run gave 781, 780 passed and one ignored, at a load average of 3.9 to 4.8: 776 at `a98235c` plus the five tests added after the fourth verification, T28–T32, all in `tests/context.rs`; the crash row's test, T26 and T27 are extended. At `a98235c`, with `57e6e1d`'s docs in place, the same run gave 775 passed and one ignored at a load average of 3.9 to 5.4: 771 at `fa6b3bf` plus T23–T27. At `fa6b3bf`, with `7119cd5`'s docs in place, the same run gave 770 passed and one ignored at a load average of 3.9 to 6.1: 767 at `448b3c2` plus T19–T22. At `448b3c2`, with `a21a7ae`'s comment edits in place, it gave 766 passed and one ignored at a load average of 4.2 to 5.5: 764 at `f9980b3` plus T16–T18. At `f9980b3` it gave 763 passed and one ignored at a load average of 3.4 to 4.4: 758 at `07bc1ea` plus T10–T15. At `07bc1ea` it gave 750 passed, one ignored and seven failed, every one a keychain test on the fake tool's `TimedOut` at a load average of 21 to 31, VERIFICATION's named load flake; run alone straight afterwards, all 19 keychain tests passed. `main`'s count at `651c86e` was 747 over 43, and this change adds thirty-six: T1–T32 and U1 in `cbr-provider`, the sixth verification's two ([above](#after-the-sixth-verification)), and J in `cbr-cli`; E1 and E2 are edits.
- **The seven conformance suites, at `a98235c`**, not run again since: nothing but `tests/context.rs` and Markdown changed after it. They were run at `a98235c` because `5891542` changes when a packet leaves the provider. They ran into `mktemp -d` with the worktree's `target/` a symlink to the worktree's build directory for the length of the run, removed afterwards, and the runner rebuilt first from the cached, checksum-verified release archive (`build_runner.py --offline`). All seven matched their expectations at the first run, at a load average of 4.8 to 5.3: `stream` 24 of 24; `core` 130 and 5 unsupported; `socket` 11 and 2; `evidence` 16; `knowledge` 10; `context` 11; `composition` 3 and 11, among the three `packets-are-sealed-at-a-separate-evidence-provider`, which publishes through an evidence peer. No fixture reaches the guard. The fifth verification's code verifier ran `context` and `composition` again at `48104d4`, in its private copy, and both matched their expectations: 11 of 11, and 3 with 11 unsupported. *(Until fix round 4 they had last run at `07bc1ea`, where `stream`, the first suite, gave 23 and one timeout on its first fixture at the first launch of a freshly built provider, at a load average near 20, and 24 of 24 when rerun alone; that was observed twice, here and in the first independent verification's run.)*

**Next.** The owner's rulings of 2026-09-26 are recorded in [m5 READINESS](m5/READINESS.md) items 7 to 18: m5b's three questions are answered (items 8 to 10), m5-arith goes before m5b, m5-settle before m5h's live run, and CTX-18/19 after m5e. The pre-existing defect of follow-up 8, a tick's commit lost after a peer's seal, by a kill or by a failure of the commit, gets a slice of its own, listed in m5 READINESS as a pending defect fix. *(Fixed since, by the peer-capture change above.)*

## Earlier — J2 live, rerun and recorded: passed

On `m5/j2-rerun`, off `main` at `80cdfbe` (#44, m5a-3), and merged with `main` at `2f8c666` (#45, m5g) before its pull request — a merge and not a rebase, so that the rubric commits this record cites keep their hashes. Rubric v1.1 and the rerun's ceilings were committed and pushed first (`8d08bef`, pushed at 22:57:14Z UTC on 2026-09-25, 04:27 IST on the 26th); the rerun followed at 22:57:23Z.

- **Rubric v1.1**, v1's mechanical changes for the `/2` projection, plus two robustness changes in its own path that did not fire ([v1.1/RUBRIC.md](../verification/j2-live/v1.1/RUBRIC.md)). Its first independent check found that a run asking nothing was judged from the harness's report alone — a usage row written into the store passed — so v1.1 now reads the store too; a second check with seventeen attacks of its own cleared it.
...
- **The path, proven** by `prove_path.py` on copies: 188 excerpts equal the fetched bytes, no `model_calls` row in the red and green stores, three in each core store answered by the configured model, no key-shaped string in any of 56 files.

**Next.** m5b, the loop and its checkpoints, which is being built. Three of its choices are the owner's — how a request starts a loop, what J4's "deduplication disabled" control means, and how CBR's own internal commands are authorised. m5b is built on recommended answers, its change records its plan and the three questions in READINESS, and it does not merge before the owner rules. The loop's live run, once m5h has built and dry-run its harness, falls under the standing grant of [m5 READINESS §10](m5/READINESS.md#10-live-calls-each-planned-estimated-and-capped-before-it).

*Note, 2026-09-26, the paragraph above kept as written:* the owner has since ruled on all three, in [m5 READINESS](m5/READINESS.md) items 8 to 10: a loop is started by the `cbr/loop` extension, listed in `requires`; J4's control is worded as question 2 recommended; and CBR's own internal commit commands are admitted within the job's resolved view and readable evidence, with no grant minted.

## Earlier — m5g, the composition feasibility check

On `m5g/composition-check`, first off `main` at `5ddc26f`, then, once its verification had cleared it, rebased onto `80cdfbe` (#44, m5a-3). Docs, one descriptor, one launch script and one `build_runner.py` flag, and after the verification round one Rust test file for those scripts. **No product Rust changed, and no model has been called.** The question came from [READINESS §7](m5/READINESS.md#7-j3-j4-and-j5-and-the-fixtures-that-need-an-execution-peer): can the pinned runner launch one composition participant from a different implementation than the others, with CBR as the context and evidence provider and the protocol's reference provider as the executor?
...
- **The rebase onto `80cdfbe`.** It conflicted in three places in VERIFICATION — the count row, taken from `main` with this change's figures brought forward, and the `check_results.py` row and "What is and is not established", each `main`'s text with this change's sentences added — and in STATE's top section, where `main`'s became *Earlier — m5a-3*; every file outside `docs/` has exactly the diff it had against `5ddc26f`. At the rebased head: a whole-workspace `cargo test --workspace --locked --no-fail-fast`, **747 tests over 43 result lines, 746 passed, one ignored, none failed** — `main`'s 730 over 42 and this binary's 17; the seven standard suites, each into its own `mktemp -d`, each matching its expectation (stream 24 pass; core 130 pass, 5 unsupported; socket 11 pass, 2 unsupported; evidence 16; knowledge 10; context 11; composition 3 pass, 11 unsupported); and the reference-executor composition run unchanged, fixture for fixture: 11 pass, `blocked-consumer-releases-the-slot-for-preparation` and `preparation-runs-without-the-consumer-slot` fail, `thirdparty-publisher-feeds-reference-verification` unsupported.
...
*Answered 2026-09-26, the block above kept as written:* by the owner's decisions of that day, in [m5 READINESS](m5/READINESS.md), the runner issue was filed as [Combraton/protocol#18](https://github.com/Combraton/protocol/issues/18), on this session's reading of the owner's reply (item 18), and CTX-18/19 go after m5e (item 17).
...
- **Follow-ups, not fixed here.**
  ...
  2. The `context.script` door does not validate ids; the guard now catches a bad one at publication, and since *Earlier — packet_invalid*, above, ends its job.
