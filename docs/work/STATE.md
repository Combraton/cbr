...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`: 793 tests over 43 `test result` lines, 792 passed, one ignored (the stall harness), none failed**, at a load average near 10, with no load flake. By crate: 20, 7, 40, 538 and 188; `tests/context.rs` 54 of 54. `main` at `4dcfb04` had 783 over 43; this change adds ten tests, all in `tests/context.rs`.
- **The seven conformance suites**, with the worktree's `target/` a symlink to its build directory only for the run, the runner built by `build_runner.py --offline`, all matching their expectations at the first run: `stream` 24; `core` 130 and 5 unsupported; `socket` 11 and 2; `evidence` 16; `knowledge` 10; `context` 11; `composition` 3 and 11 unsupported, `packets-are-sealed-at-a-separate-evidence-provider` among the three passes.
