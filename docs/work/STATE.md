...
On `m5a-3/floor`, first off `main` at `041ad5f` and rebased onto `2eedade`, which holds J2 live's record (#41) and `cbr expand` (#42). The builder's four commits are now `0073c33` (tests first), `479bcdf` (the floor), `2168435` (tests for the survivors) and `b70c1fc` (code docs and counts); before the rebase they were `2c790c3`, `02663a9`, `28383ce` and `4621f29`. Three independent verifiers then read `4621f29`. The fix round for their findings is `1648d8b` (tests first), `ef2b2d0`, `c9c739a`, `73bd7e4` and `e01016b` (the fixes), `e32d57b`, `2bd43e8` and `9654d4b` (tests for three mutants the fixes let survive: M4, C2b and J3), and this docs commit. **No model has been called.** The rule is [READINESS §3](m5/READINESS.md#3-journey-2-first) and [ADR 001's question 16](../decisions/001-standalone-v0.1-scope-and-stack.md); the rerun's plan is [READINESS §10](m5/READINESS.md#j2-live-rerun-after-m5a-3-inputs-estimate-and-ceiling).
...
- **Unchanged.** `cbr-context-compiler/3` stays: the compiler's golden packet has no projection. The golden projection digest is re-pinned: with `/1`, no F1 line and the planned part count put back, the new code renders the old digest byte for byte.
...
**109 mutants at the head, 104 killed.** They are:
...
| C-binary | `partition`'s early return without the not-text check | survived targeted; whole workspace: survived (`idle_connections_keep_at_least_half_of_command_throughput` failed, the load flake, discounted) | — |
...
**The five survivors, kept, each with its reason:**
...
- **C-binary** (`partition` without its not-text check) is **equivalent**. A read that is not text has no units, so `offer`'s first drawing fits and it returns `None` at once.
...
4. **The floor's greedy pass draws once per failure or identity unit.** 8,700 identity lines of `running 1 test` took 0.65 s in a debug build (scratch probe, reverted). That is within capacity and bounded by `INPUT_BYTES`. It is not the O(units²) that capacity-first fixed, which was unbounded, and it is noted rather than changed.

### Gates at the head

At `9654d4b`, the last code commit; this docs commit changes only Markdown.
...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`: 695 passed, 0 failed and 1 ignored (the stall harness), over 42 `test result` lines.** By crate: 20, 7, 40, 466 and 162. That is 670 over 42 on `main` at `2eedade`, plus twenty-five: the builder's eighteen and the fix round's seven. No timing test failed in that run. In the survivors' whole-workspace runs, `idle_connections_keep_at_least_half_of_command_throughput` failed once and was discounted by name; in the targeted runs, `keychain::tests` failed with `TimedOut`.
- **The seven conformance suites**, run into `mktemp -d` with the worktree's `target/` a symlink for the length of the run and removed afterwards. Each matches its expectation:
  - `stream` 24;
  - `core` 130 and 5 unsupported;
  - `socket` 11 and 2;
  - `evidence` 16;
  - `knowledge` 10;
  - `context` 11;
  - `composition` 3 and 11.
- **The dry-run survey**, of all nine inputs and both live manifests at `e01016b`, whose binaries are this head's: the tests added since change no binary. It is [READINESS §10](m5/READINESS.md#j2-live-rerun-after-m5a-3-inputs-estimate-and-ceiling)'s table. Every run exited 0 with no problem named. Only the core manifest asks, 3 parts. Red and green make no call, and each assisted section is its baseline's byte for byte. The masked baseline digests equal the builder's and a verifier's surveys at `4621f29`, so the fix round changed no baseline.
