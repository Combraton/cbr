...
- (d) A valid revision published in the same tick as the refusal is dropped with that tick, and never sealed, here or at an evidence peer. Only a script can do this: `a_refusal_commits_nothing_of_the_tick_that_was_refused`, and under a peer `no_packet_of_a_refused_tick_that_publishes_twice_reaches_the_evidence_peer` (T24). So is another subscriber's valid packet in that tick, (b).
...
5. **An issue to the protocol, not filed**: CONTEXT §3 could say that `refused` may follow `preparing`, with a `job`. Filing it is the owner's call; protocol#18 was filed on this session's reading of the owner's reply ([m5 READINESS](m5/READINESS.md) item 18).
...
8. **A pre-existing defect, not fixed here: a kill after an evidence peer's seal and before the batch commits leaves the request `preparing` for ever once the clock moves.** It is not this change's, and gets a slice of its own; [m5 READINESS](m5/READINESS.md) lists it as a pending defect fix. It has been there since m3a: `publish_one` at `651c86e` already sent a packet to the peer before its tick's batch committed, and kept a capture instant only on a send that failed. A kill between the peer's seal and the commit keeps nothing, so nothing in CBR names the packet sealed at the peer. Restarted at the same instant, the retry describes the packet exactly as before, the peer replays its own steps, and the request is published. Once the clock has moved, the retry describes it with a new `captured_at`; the peer refuses the same idempotency key with a different digest as `idempotency_conflict`, and the new instant is kept, so every later tick fails the same way, past the deadline too, whose publication goes the same way. The fourth verification's code verifier showed it with a probe, in its private copy only, which added a barrier after each peer send (`context.packet.after_peer_sealed`, in no source here) and killed the provider there: with the clock moved five minutes before the restart, the request read `preparing` after ten inspects and again past its deadline, the log held 13 lines *packet packet.r-1.1 not sealed at the evidence provider (refused: idempotency_conflict)*, the peer held `packet.r-1.1` `sealed` with `captured_at` 00:00, and the job kept `captures` `{"packet.r-1.1":"2030-01-01T00:05:00Z"}`; with the clock unmoved, the restart published it `ready`, under the peer's digest. Fixing it changes provider code, which this round does not: the capture instant would have to be durable before the send, or the retry would have to recognise the peer's seal as its own.

### Gates at the head

At `6deafa9`, the last commit with tests, with this docs commit's edits in place; this docs commit changes Markdown only. Provider code changed in `5891542`, the fourth fix round's fix, and nowhere else since `104615d` but `448b3c2`'s doc comment; `6deafa9` changes `tests/context.rs` alone.
