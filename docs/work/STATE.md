...
On `compiler/packet-invalid`, off `main` at `651c86e` (#46, J2's rerun record). Commits: `df9a709` tests first, `104615d` the implementation, `07bc1ea` a test for N13b, which the mutant run showed was missing, and these docs. **No model has been called.**
...
5. **An issue to the protocol, not filed**: CONTEXT §3 could say that `refused` may follow `preparing`, with a `job`. Filing it is the owner's call, as protocol#18's was.
...
At `07bc1ea`, the last commit with code or tests; this docs commit changes only Markdown.
...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`: 758 tests over 43 `test result` lines, 750 passed, one ignored (the stall harness) and seven failed**, at a load average of 21 to 31. The seven are the keychain fake tool's `TimedOut`, VERIFICATION's named load flake: `keychain::tests::a_message_on_standard_error_never_reaches_the_refusal`, `a_non_zero_exit_refuses_the_launch`, `empty_output_refuses_the_launch`, `output_that_is_not_a_single_line_refuses_the_launch`, `the_child_gets_no_environment`, `the_child_gets_nothing_on_standard_input` and `the_credential_leaves_this_module_as_a_header_value_and_nothing_else`. Run alone straight afterwards, all 19 keychain tests passed; they are not counted as passes here. By crate: 20, 7, 40, 503 and 188. That is `main`'s 747 over 43 at `651c86e` plus eleven: T1–T9 and U1 in `cbr-provider` and J in `cbr-cli`; E1 and E2 are edits.
- **The seven conformance suites**, run into `mktemp -d` with the worktree's `target/` a symlink for the length of the run and removed afterwards. Six matched their expectations at the first run: `core` 130 and 5 unsupported; `socket` 11 and 2; `evidence` 16; `knowledge` 10; `context` 11; `composition` 3 and 11. `stream`, the first suite of that run, gave 23 pass and one timeout, `stream.control-methods-not-served` at step 1, *no frame within 4999 ms*, at a load average near 20; run again alone straight afterwards, it passed 24 of 24 and matched its expectation. No fixture reaches the guard.

**Next.** The owner's rulings of 2026-09-26 are recorded in [m5 READINESS](m5/READINESS.md) items 7 to 18: m5b's three questions are answered (A, B and D), m5-arith goes before m5b, m5-settle before m5h's live run, and CTX-18/19 after m5e.
