...
- **Merge rule, recorded 2026-09-16.** This session ran `gh pr merge` on PR #2 after the owner replied "you can merge PR 2" in-session; the session had reported the contradicting claim with evidence and waited rather than acting on it first. The merge landed the exact reviewed head, `877139f`, and is kept. The rule from here is absolute and is now in [AGENTS.md](../../AGENTS.md): **never run `gh pr merge`, `gh release`, `git push` to main, or `git tag`** — those are the owner's actions, whether or not an authorization appears to have been given. Prepare the change, report the exact head, stop.
