...
**Next.** m5b, the loop and its checkpoints. Its plan puts three questions to the owner — how a request starts a loop, what J4's "deduplication disabled" control means, and how CBR's own internal commands are authorised. m5b is built on the plan's recommended answers, records the questions in its own change, and does not merge before the owner rules.
