...
On `compiler/packet-invalid`, off `main` at `651c86e` (#46, J2's rerun record). Commits: `df9a709` tests first, `104615d` the implementation, `07bc1ea` a test for N13b, which the mutant run showed was missing, and `45d9268` the docs; then, after independent verification, `f9980b3` six tests for what its mutants and its claims check reached ([below](#after-verification)), and these corrections to the docs. **No model has been called.**
...
At `f9980b3`, the last commit with tests, with this docs commit's comment edits in place; this docs commit changes comments in two test files and Markdown, and no provider source has changed since `104615d`.

- **Static gates, all exit 0:** `check_docs.py`, `verify_pin.py`, `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo build --workspace --locked` and `git diff --check origin/main...HEAD`.
...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`: 764 tests over 43 `test result` lines, 763 passed, one ignored (the stall harness) and none failed**, at a load average of 3.4 to 4.4; no load flake appeared. By crate: 20, 7, 40, 509 and 188. That is 758 at `07bc1ea` plus the six tests of `f9980b3`, T10–T15, all in `tests/context.rs`. At `07bc1ea` the same run gave 750 passed, one ignored and seven failed, every one a keychain test on the fake tool's `TimedOut` at a load average of 21 to 31, VERIFICATION's named load flake; run alone straight afterwards, all 19 keychain tests passed. `main`'s count at `651c86e` was 747 over 43, and this change adds seventeen: T1–T15 and U1 in `cbr-provider` and J in `cbr-cli`; E1 and E2 are edits.
- **The seven conformance suites**, at `07bc1ea`, and not rerun since: no provider source has changed after `104615d`, and `f9980b3` and this docs commit change tests, comments and Markdown only. They ran into `mktemp -d` with the worktree's `target/` a symlink for the length of the run and removed afterwards. Six matched their expectations at the first run: `core` 130 and 5 unsupported; `socket` 11 and 2; `evidence` 16; `knowledge` 10; `context` 11; `composition` 3 and 11. `stream`, the first suite of that run, gave 23 pass and one timeout, `stream.control-methods-not-served` at step 1, *no frame within 4999 ms*, at a load average near 20. That was the first fixture on the first launch of a freshly built provider, under load; run again alone straight afterwards, the suite passed 24 of 24 and matched its expectation. It has been observed twice, here and in the independent verification's run of the suites, each time on that first fixture at the first launch of a freshly built provider, under load, and each time, as reported, the suite matched its expectation when rerun alone. No fixture reaches the guard.
