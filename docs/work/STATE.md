...
At `6c894fd`, the last code commit; this docs commit changes only Markdown.
...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`: 726 passed, 0 failed and 1 ignored (the stall harness), over 42 `test result` lines**, at a load average near 6. By crate: 20, 7, 40, 490 and 170. That is `main`'s 694 over 42 at `5ddc26f`, plus thirty-three: the builder's eighteen, the first fix round's seven and this round's eight, all in `projection::tests` but four in `journey_two` and two in `j2_harness`. Right after the rebase, at `7ad9fae`, the same run gave 718 passed, 0 failed, 1 ignored over 42 lines, and at `bd8791a`, the fix, 725. No timing test failed in any of the three, nor in CA-true's whole-workspace run.
