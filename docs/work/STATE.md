...
- **A pre-existing defect**, which this change neither introduced nor fixes: a kill after an evidence peer's seal and before the batch commits leaves the request `preparing` for ever once the clock moves. It is follow-up 8 below, with the probe that showed it, and gets its own slice.
...
8. **An evidence peer's seal cannot be rolled back, and two things follow from it.** Neither is this change's defect. The first gets a slice of its own, which [m5 READINESS](m5/READINESS.md) lists as a pending defect fix; the second is a limit of the guard, recorded here and in choice (d).
   - **A pre-existing defect, not fixed here: a tick whose batch fails to commit after an evidence peer has sealed its packet leaves the request `preparing` for ever once the clock moves.** A kill between the peer's seal and the commit does it, and so does any failure of the commit itself, such as SQLite busy past the provider's 10-second timeout or a full disk. It has been there since m3a: `publish_one` at `651c86e` already sent a packet to the peer before its tick's batch committed, and kept a capture instant only on a send that failed. When the commit is lost after a send that sealed, nothing is kept, so nothing in CBR names the packet sealed at the peer. Restarted after a kill at the same instant, the retry describes the packet exactly as before, the peer replays its own steps, and the request is published. Once the clock has moved, the retry describes it with a new `captured_at`; the peer refuses the same idempotency key with a different digest as `idempotency_conflict`, and the new instant is kept, so every later tick fails the same way, past the deadline too, whose publication goes the same way. The fourth verification's code verifier showed it with a probe, in its private copy only, which added a barrier after each peer send (`context.packet.after_peer_sealed`, in no source here) and killed the provider there: with the clock moved five minutes before the restart, the request read `preparing` after ten inspects and again past its deadline, the log held 13 lines *packet packet.r-1.1 not sealed at the evidence provider (refused: idempotency_conflict)*, the peer held `packet.r-1.1` `sealed` with `captured_at` 00:00, and the job kept `captures` `{"packet.r-1.1":"2030-01-01T00:05:00Z"}`; with the clock unmoved, the restart published it `ready`, under the peer's digest. The fifth verification's code verifier showed it with no kill (probe P2, in its private copy): with the provider up, a second connection held SQLite's write lock (`BEGIN IMMEDIATE`) past the busy timeout while a tick at 00:00 sealed `packet.r-1.1` at the peer, so that tick's commit failed (*store: sqlite: database is locked*, then *context preparation failed: unavailable*). With the lock released and the clock at 00:05, the request read `preparing` after ten inspects, and again after five more at 00:31, past its 00:30 deadline; the log held 15 lines *packet packet.r-1.1 not sealed at the evidence provider (refused: idempotency_conflict)*, the peer held `packet.r-1.1` `sealed` with `captured_at` 00:00, and the job kept `captures` `{"packet.r-1.1":"2030-01-01T00:05:00Z"}`. Fixing it changes provider code, which this change does not: the capture instant would have to be made durable before the send, or the retry would have to recognise the peer's seal as its own. Either covers a kill and a failed commit alike, since neither depends on how the commit was lost. *(Until the fifth verification this item named a kill alone.)*
...
- **Follow-ups, not fixed here.**
  ...
  2. The `context.script` door does not validate ids; the guard now catches a bad one at publication, and since *This change — packet_invalid*, above, ends its job.
