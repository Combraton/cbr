...
- (d) A valid revision published in the same tick as the refusal is dropped with that tick. Only a script can do this: `a_refusal_commits_nothing_of_the_tick_that_was_refused`. So is another subscriber's valid packet sealed earlier in that tick, (b) and T19.
...
7. **Z22 is not pinned.** A subscriber cancelled before a `packet_invalid` ending reads its items off the ended job's stored sections, so clearing them is observable, and no test holds it ([above](#after-the-third-verification)).

### Gates at the head

At `fa6b3bf`, the last commit with tests, with this docs commit's edits in place; this docs commit changes Markdown only. No provider code has changed since `104615d`: `448b3c2` changes one doc comment, and every later commit tests, test comments or Markdown.
...
- **`cargo test --workspace --locked --no-fail-fast`, through `wslot`, after `cargo build --workspace --locked`: 771 tests over 43 `test result` lines, 770 passed, one ignored (the stall harness) and none failed**, at a load average of 3.9 to 6.1; no load flake appeared. By crate: 20, 7, 40, 516 and 188. That is 767 at `448b3c2` plus the four tests of `fa6b3bf`, T19–T22, all in `tests/context.rs`, which is 32 of 32. At `448b3c2`, with `a21a7ae`'s comment edits in place, the same run gave 766 passed and one ignored at a load average of 4.2 to 5.5: 764 at `f9980b3` plus T16–T18. At `f9980b3` the same run gave 763 passed and one ignored at a load average of 3.4 to 4.4: 758 at `07bc1ea` plus T10–T15. At `07bc1ea` the same run gave 750 passed, one ignored and seven failed, every one a keychain test on the fake tool's `TimedOut` at a load average of 21 to 31, VERIFICATION's named load flake; run alone straight afterwards, all 19 keychain tests passed. `main`'s count at `651c86e` was 747 over 43, and this change adds twenty-four: T1–T22 and U1 in `cbr-provider` and J in `cbr-cli`; E1 and E2 are edits.
- **The seven conformance suites**, at `07bc1ea`, and not rerun since: no provider code has changed after `104615d`, and `f9980b3`, `fb39ca1`, `6e3f838`, `448b3c2`, `a21a7ae`, `fa6b3bf` and this docs commit change tests, comments and Markdown only. They ran into `mktemp -d` with the worktree's `target/` a symlink for the length of the run and removed afterwards. Six matched their expectations at the first run: `core` 130 and 5 unsupported; `socket` 11 and 2; `evidence` 16; `knowledge` 10; `context` 11; `composition` 3 and 11. `stream`, the first suite of that run, gave 23 pass and one timeout, `stream.control-methods-not-served` at step 1, *no frame within 4999 ms*, at a load average near 20. That was the first fixture on the first launch of a freshly built provider, under load; run again alone straight afterwards, the suite passed 24 of 24 and matched its expectation. It has been observed twice, here and in the independent verification's run of the suites, each time on that first fixture at the first launch of a freshly built provider, under load, and each time, as reported, the suite matched its expectation when rerun alone. No fixture reaches the guard.
