...
1. **Two refused shapes.** A request refused at submit (`budget_insufficient`) has no `job` and its items read `pending`, since inspect falls back to `item_results` over no job; a `packet_invalid` one has its `job` and items `unmet` or `degraded`. Both are schema-valid.
