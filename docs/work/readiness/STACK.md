...
The piece most likely to be assumed to need async — bounding "produced but not yet written" bytes per connection — is 156 lines of `Mutex` + `Condvar` + `VecDeque` with a dedicated writer thread. The session thread queues and reads `pending`, and never performs a write. **Corrected in M1 stage (c4):** it does wait on a consumer that stopped reading, for the room deadline and no longer — on one connection that wait *is* the backpressure, since a consumer that has stopped reading should not be sent more nor have its further requests read. What it never does is wait inside a write or wait past the deadline, and a command's commit never waits on output at all (`crates/cbr-provider/src/session.rs`).
...
| **CBR's envelope** | **20M tokens per 5-hour window and 200M per month**, confirmed by the owner 2026-09-16 (ADR 001). Background spend is **zero** until explicitly enabled. **This row is the single source for both numbers** until M4 introduces the envelope configuration, which is seeded from it. |
