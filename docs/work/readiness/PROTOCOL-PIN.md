...
| `tree` (git) | The **root tree object id** of the commit, `git rev-parse <commit>^{tree}` — not the commit id. | Two commits with identical content are the same content basis, and applicability conditions are about content. Cost: the commit is no longer recoverable from the condition; it must travel as evidence or as a claim statement if it matters. Submodule contents are covered by the tree; submodule *commits* are recorded as gitlink entries only. |
...
| `build` | Same shape as `environment`. See G1: it cannot be a condition in 0.1. | — |
