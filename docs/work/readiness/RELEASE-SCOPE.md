...
**m5g took the first option on 2026-09-26** ([READINESS §7](../m5/READINESS.md#7-j3-j4-and-j5-and-the-fixtures-that-need-an-execution-peer)). The pinned runner starts every participant from one descriptor. [`cbr-with-reference-executor-unix.json`](../../../conformance/participants/cbr-with-reference-executor-unix.json) launches a small script that starts the reference provider for a named composition participant configured as an executor and CBR for every other participant, with no runner change. It refuses an executor configuration anywhere else, and `run_fixtures.py` refuses the descriptor outside the composition suite, so its `execution/1` claim never reports a reference result under CBR's name. **8 of the ten pass. 2 fail on CBR's side**: CBR's context provider does not yet submit the investigation execution that CTX-18 and CTX-19 expect of it, which is CBR as an `execution/1` client, not a provider. The reference provider is built from the verified release archive and never vendored or linked. The verification fixture stays out of reach: its one participant would have to serve `verification/1` itself.
