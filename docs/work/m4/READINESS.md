...
**Nor does reading it write to it** — changed at m4h. Until then the harness read the spend and the records over a read-write connection, which on closing folded each store's log into its database and deleted the log. That is why no store of live runs 1 to 3 has a log beside it: a killed provider never closes its connection and so cannot delete its own, and the harness's was the last to close. Every committed row is intact; the files are not as the provider left them. Measured on a store whose provider was killed with its ledger rows still in the write-ahead log, which is how every run leaves one: a read-write connection checkpoints on close, folding the log into the database and deleting the log and the shared-memory file; `mode=ro` reads the log but rewrites the shared-memory file, and on a store with no log creates both; `immutable=1` changes nothing and misses the log. So a store with a log is read `mode=ro` with a read-only shared memory, which reads the log and writes nothing; one with no log has nothing `immutable=1` could miss; and a log with no shared-memory file is refused rather than written to. Two tests hold both states to a byte comparison of every file.
