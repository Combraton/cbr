...
**Since m5-settle (2026-09-26) a count reserves the completion body's input bound**, where its settlement is capped, rather than its own body's, so it never settles above what it reserved ([ADR 001](../../decisions/001-standalone-v0.1-scope-and-stack.md) question 15, amended).
...
Three stops since m5-settle, in the order a call ends on them: a charge above the **local bound** records `bound_unsound` and stops everything, because the bound is wrong; a bill above the call's **reservation** records an `overrun` naming its job and request and stops everything; and, on the serving count path only, input billed above the **count** a completion was admitted on by more than the margin of 1,024 records `count_unsound` and closes the count path, where until m5-settle it was a finding. The first two are read inside `Ledger::admit`, before every ceiling. Under `Counting::Always` a bill above the count stays the calibration's finding (§10).
...
- **A reservation is written before the send and reconciled after.** A crash between the two therefore leaves the spend **counted rather than forgotten** — the conservative direction. Reconciliation replaces the reservation with the response's actual usage, charged whole. Since m5-settle a bill above the reservation is recorded in the same transaction as an `overrun` naming its job and request, and the store admits nothing again; stores written before it hold unattributed `divergence` rows, which stop nothing.
...
**What one question can hold on the serving path**, as m5-arith states it. A question is at most two attempts, its send and one repair ([`model::REPAIRS`]), and each attempt is at most two sends, a count and a completion, so four sends in all. It holds less than four sends' reservations, because of when a count is made. The local bound admits first. A count is made only after the local bound is refused on the job, the window, the month or the run — a counter a tighter figure could satisfy — and never after a per-request refusal (`model::SERVING_COUNTING`, which both serving launches name and a source test holds them to). The count and the completion admitted on it are then both admitted on the counter that refused the send, so together they hold less than the send would have — and since m5-settle, if room was freed on that counter meanwhile, the completion is refused when the two would pass that send. So a question holds at most **its first send and its widest repair**, which is what the table prices. `model::tests::no_serving_question_holds_more_than_question_worst` measures that on the real call path, over a grid of ceilings around each body's reservations, rather than arguing it.

**What that assumes**, in the words of `model::question_worst`'s own documentation:

> **What it assumes. Only that bills are within their reservations**: the byte bound holds, a completion admitted on a count is billed input within the count and the margin, `max_output_tokens` is honoured, and a failed call reports no more than it reserved.
>
> **Room freed changes nothing.** A count reserves the completion body's input bound, where its settlement is capped. A completion is admitted on a count only when the count's charge and its own reservation fit within the send the local bound refused. So an attempt reserves at most `W` however room is freed.
>
> **A bill above its reservation** is charged whole, recorded as an `overrun` naming its job and request, and ends its call, and the store admits nothing again. So a question can hold more than this only by what a provider billed above a reservation, and only calls already admitted when the first overrun settles can add to it.
>
> **Under `Counting::Always`** the believed count is capped at the local bound, so an attempt reserves at most `local + W`. This function does not price that, because no serving launch counts that way.

**Freed room is closed by the per-attempt rule** (m5-settle). `Runtime::call` admits the count and then the completion in two separate admissions, so room freed on the refusing counter in between — while the count is admitted, on the wire, answered or being settled, or after it — used to let an attempt hold more than its send `W`, up to `W + Ic − (local − Ic)` with an honest count. Now a completion admitted on a count is refused, and the local refusal stands, when the count's charge and its own reservation would pass `W`; an `attempt_over_send` note records the sum. `model::tests::a_count_path_completion_is_refused_when_its_count_and_its_reservation_would_pass_the_send` frees the room at each place the call path names with every honest count, `the_per_attempt_rule_admits_exactly_the_send_and_refuses_one_token_more` pins the boundary, and `no_serving_question_holds_more_than_question_worst_whatever_room_is_freed` adds ceilings up to two counts past `question_worst`.

**Under `Counting::Always`** the count comes first, and since m5-settle the count it believes is capped at the local bound, so an attempt reserves at most `local + W`, which `question_worst` does not price. Only the calibration counts that way, and the source test above holds serving to `SERVING_COUNTING`.

**Settlement: route 3 and freed room are closed; routes 1, 2 and 4 are caught at the first occurrence, after which the store admits nothing.** Every ceiling holds at admission, and a call settles at what the provider billed. Four routes could put a bill above its reservation:

1. a completion admitted on a count is billed above the count;
2. the input is billed above the byte bound, which also records `bound_unsound`, or the output above `max_output_tokens`;
3. a count settles above what it reserved: closed by m5-settle, since a count now reserves the completion body's input bound, where its settlement is capped;
4. a failed call reports usage above its reservation.

Routes 1, 2 and 4 are a provider billing above what it was asked for, which nothing CBR sends can prevent. Since m5-settle each is charged whole, recorded as an `overrun` naming its job and request, and ends its call `reservation_overrun`, and the store admits nothing again: `model::tests::a_bill_above_its_reservation_ends_its_call_and_nothing_after_it_is_admitted` asserts each, with a fresh runtime over the same store. So a store can pass a ceiling once, by what the calls in flight when the first overrun settles bill above their reservations: at most `work::CONCURRENCY` (2) pool units, plus any unit detached at its deadline and still running. Route 1's excess is at most `local − floor − 1,024` per call, pinned by the same test at 52,015, 165,527, 70,782 and 32,366 on the widest terms, choice, selection and J2-part bodies; routes 2 and 4 have no bound CBR can derive. A stop is cleared with a new data directory ([ADR 001](../../decisions/001-standalone-v0.1-scope-and-stack.md) question 18).

**A settlement that does not land** (m5-settle's verification round). An overrun the store refuses to take — another writer holding the lock past the busy timeout, an I/O error, a full disk — is kept by the process, which refuses every later admission on that store and writes it at the first admission the store takes (`model::tests::an_overrun_whose_settlement_the_store_refused_still_stops_the_store`); the call's charge is the bill. A process killed after the answer was recorded and before its settlement landed is reconciled at the next start: the recording boundary writes each exchange beside the reservation it was sent under, and a recorded bill above the reservation is charged and writes its `overrun` before anything is admitted (`a_store_killed_before_it_settled_an_overrun_is_stopped_when_it_is_opened_again`, and the journey `model_selection::a_provider_killed_before_it_settled_an_overbill_is_stopped_when_it_starts_again`). A serving start does this, and so does the calibration launch, which no test reaches. **Round 2:** the process's stop is read under the admission's lock (`an_admission_already_waiting_on_the_lock_is_refused_once_the_process_keeps_an_overrun`); the live-run harnesses reconcile each killed launch's store with `cbr-provider --reconcile-ledger` before reading it, and go no further when that fails (`an_overrun_only_the_killed_launch_knew_of_still_ends_the_sequence` in `m4e_harness.rs` and `j2_harness.rs`, and `the_harness_goes_no_further_when_a_store_cannot_be_reconciled`); and an answer the transport read as the provider's quota being gone names no reservation in its record, so a start does not charge a 429's body (`a_start_settles_no_reservation_its_record_does_not_bill_above`). **Round 3:** a refused overrun is kept before the ROLLBACK that frees the settlement's lock (`budget::tests::an_admission_waiting_on_a_refused_settlements_own_lock_is_refused`), and `finish` charges from what `settle` says it kept. **Not closed:** a call killed before its answer was written leaves nothing to reconcile (`a_call_killed_before_its_answer_was_recorded_leaves_no_stop`), and only an overrun is kept and reconciled (question 18).
...
| **m5-settle: a bill above its reservation clamped, or its `overrun` row unattributed** | `budget::tests::a_settlement_above_its_reservation_is_charged_whole_and_is_an_overrun_naming_its_job_and_request` |
| **m5-settle: the stop ignored by admission, keyed on `divergence`, scoped to one job, or read after the ceilings** | `a_store_that_recorded_an_overrun_admits_nothing_again_even_after_a_restart`, `an_old_divergence_row_stops_nothing`, `a_bill_above_its_reservation_ends_its_call_and_nothing_after_it_is_admitted`, and the journey `a_provider_billing_past_its_reservation_stops_the_store_for_good` |
| **m5-settle: the settlement and its `overrun` row written apart** | `a_settlement_whose_overrun_row_cannot_be_written_leaves_its_reservation_standing` |
| **m5-settle's verification round: an overrun the store refused forgotten by the process, or a killed call's recorded bill not reconciled at the next start** | `an_overrun_whose_settlement_the_store_refused_still_stops_the_store`, `a_store_that_cannot_take_an_overrun_is_stopped_in_this_process_and_reports_its_bound_first`, `a_store_killed_before_it_settled_an_overrun_is_stopped_when_it_is_opened_again`, `a_start_settles_no_reservation_its_record_does_not_bill_above`, and the journey `a_provider_killed_before_it_settled_an_overbill_is_stopped_when_it_starts_again` |
| **m5-settle's verification round 2: the process's stop read before the admission's lock, a harness reading a killed launch's store unreconciled or going on when it cannot be reconciled, an exhausted answer reconciled as a bill, a start stopping at the first reservation it leaves, or a refused settlement that was no overrun kept or charged its figure** | `an_admission_already_waiting_on_the_lock_is_refused_once_the_process_keeps_an_overrun`; `an_overrun_only_the_killed_launch_knew_of_still_ends_the_sequence` (`m4e_harness.rs`, `j2_harness.rs`) and `the_harness_goes_no_further_when_a_store_cannot_be_reconciled`; `a_start_settles_no_reservation_its_record_does_not_bill_above` and `a_start_reads_every_standing_reservation_past_one_it_leaves`; `budget::tests::a_refused_settlement_within_its_reservation_is_no_stop` and `a_settlement_the_store_refused_that_was_no_overrun_is_charged_the_estimate` |
| **m5-settle: a count reserving its own body's bound, or the per-attempt rule removed or loosened** | `a_count_reserves_the_most_its_settlement_can_charge`; `a_count_path_completion_is_refused_when_its_count_and_its_reservation_would_pass_the_send` and `the_per_attempt_rule_admits_exactly_the_send_and_refuses_one_token_more` |
...
**The second comparison is a finding, never a stop.** A bill above the count endpoint's prediction says the prediction is not reliable, which is a fact about the count endpoint for the owner to weigh; it does not falsify the local bound, which is the thing every admission rests on. The two must not be confused, which is why they have different consequences written down before the run. **Since m5-settle** the count the calibration believes is capped at the local bound, and a bill above it stays this finding; on the serving count path the same comparison, past the margin of 1,024, closes the count path (§3).
