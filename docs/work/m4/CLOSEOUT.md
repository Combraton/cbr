...
The promises are [READINESS §1](READINESS.md#1-scope-and-the-promise)'s, issue #21's and [RELEASE-SCOPE §4](../readiness/RELEASE-SCOPE.md)'s M4 row. Three of them were not delivered, and they are rows here rather than omissions.
...
| Model-assisted **artifact creation** — a derivation that produces a claim or a projection | **Not delivered.** M4's derivations are the records of selection and discovery, and none of them produces a claim or a projection. READINESS §1 gives projections and the wider derivation families to M5. |
...
| RELEASE-SCOPE §4's **bounded tool surface, the loop and checkpoints** | **Not built.** Every model call in M4 is a direct call with a fixed question — the first row of [MODEL-RUNTIME §6](../../spec/MODEL-RUNTIME.md). There is no tool, no loop and no loop checkpoint. What exists of restart is the ledger's reservation and the six model rows of the crash matrix. RELEASE-SCOPE's M4 row still names all three, and moving them is the owner's decision. |
...
**One obligation M4 did not exercise.** [RELEASE-SCOPE §2](../readiness/RELEASE-SCOPE.md) gives M4 a test that CBR's own producer never names a derived artifact as an ancestry root. That obligation applies to a family whose output is cited as claim support, and no record of discovery's is cited as support, so nothing M4 built reaches it. It belongs with the families. RELEASE-SCOPE still assigns it to M4, and reassigning it is the owner's decision.
...
- **A repaired step's record carries the usage of its last exchange only.** `model::Runtime::ask` builds its `Cost` from the attempt that ended the question. The ledger holds every charge and is right; the record, read on its own, leaves out the attempt it repaired. This was found while writing run 3's record and is reported, not fixed.
- **The harness opens the evidence store read-write** to read the spend, the records and the ambiguity report. A read-only connection is for the next code change.
