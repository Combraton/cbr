...
- **The harness opens the evidence store read-write** to read the spend, the records and the ambiguity report — and on closing, that connection folded each store's log into its database and deleted the log. A killed provider never closes its own connection, so the stores of runs 1 to 3 are as the harness left them, every committed row intact. **Fixed after M4's close, at m4h**: it now reads without writing a byte.
