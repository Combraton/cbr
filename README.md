...
What **does not** exist yet: model-produced claims; a tool-using investigation loop with checkpoints; background maintenance; and a release. [VERIFICATION](docs/VERIFICATION.md) has every command and what each one does not establish. [JOURNEYS](docs/verification/JOURNEYS.md) has every journey run, the live ones included. The close-outs of [M1](docs/work/m1/CLOSEOUT.md), [M3](docs/work/m3/CLOSEOUT.md) and [M4](docs/work/m4/CLOSEOUT.md) have those milestones' outcomes, limits and live-run costs; J2's live runs, which are M5's, have theirs in [m5 READINESS §10](docs/work/m5/READINESS.md#10-live-calls-each-planned-estimated-and-capped-before-it) and JOURNEYS.
