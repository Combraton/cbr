...
> **Status, 2026-09-23: milestones M1 to M4 of standalone v0.1 are complete. Nothing is released.** CBR is a working protocol provider for evidence, for scoped, challengeable claims, and for context requests answered with exact sealed packets. A packet is compiled from a registered repository by lexical retrieval, code anchors and claim applicability. A request that authorises it adds bounded model-assisted selection and discovery, and **live-model evidence is recorded** ([M4 close-out](docs/work/m4/CLOSEOUT.md)). The reviewed architecture is `architecture-v1-20260912`, published as `public-development-v1-20260913`. Canonical specifications are available through [the documentation map](docs/README.md). This README is an overview, not the full specification.
...
What **does not** exist yet: model-produced claims and projections, and Journey 2, which needs them; a tool-using investigation loop with checkpoints; background maintenance; and a release. [VERIFICATION](docs/VERIFICATION.md) has every command and what each one does not establish. [JOURNEYS](docs/verification/JOURNEYS.md) has every journey run, the live ones included. The close-outs of [M1](docs/work/m1/CLOSEOUT.md), [M3](docs/work/m3/CLOSEOUT.md) and [M4](docs/work/m4/CLOSEOUT.md) have the outcomes, the limits, and the cost of every live run.
