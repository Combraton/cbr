...
fn the_count_call_is_a_send_and_is_admitted_recorded_and_charged_like_one() {
    ...
    let ended = runtime.call(T0, "job", "r", b"{\"messages\":[]}", 1, &no_barrier);
    assert!(
...
fn a_request_the_local_estimate_refuses_never_reaches_the_count_endpoint() {
    ...
    let huge = vec![b'x'; PER_REQUEST_TOKENS as usize + 1];
    let ended = runtime.call(T0, "job", "r", &huge, 1, &no_barrier);
...
fn an_exhausted_envelope_refuses_before_the_count_call() {
    ...
    let ended = runtime.call(T0, "fresh", "r", b"{}", 1, &no_barrier);
...
fn provider_exhaustion_reaches_the_caller_as_its_own_reason_and_is_not_retried() {
    ...
    let ended = runtime.call(T0, "job", "r", b"{}", 1, &no_barrier);
...
fn a_failed_call_reaches_the_caller_as_an_unmet_reason_and_never_hangs() {
    let connection = database();
    let transport = Recorder::new(vec![Answer::Failed("connection reset".into())]);
    let runtime = Runtime {
        ledger: Ledger::new(&connection),
        transport: &transport,
    };
...
fn a_usage_that_differs_from_the_count_is_what_the_ledger_keeps() {
    ...
    runtime.call(T0, "job", "r", b"{}", 1, &no_barrier);
    assert_eq!(
        Ledger::new(&connection)
            .spend(T0, "job")
            .expect("spend")
...
fn every_crash_boundary_is_reached_in_order() {
    ...
    runtime.call(T0, "job", "r", b"{}", 1, &|name| {
        ...
        match name {
            AFTER_RESERVATION => assert_eq!(
                sent,
                seen.iter().filter(|n| **n == AFTER_SEND).count(),
                "a reservation is written before its send, never after it"
            ),
            AFTER_SEND => assert_eq!(
                sent,
                seen.iter().filter(|n| **n == AFTER_SEND).count() + 1,
